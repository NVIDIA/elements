...
This section captures non-obvious, durable context for cloud agents. The startup update script runs `nvm use` then `pnpm install --frozen-lockfile --prefer-offline`, so dependencies, Playwright Chromium, and Vale are already installed when you start. Use the standard commands documented above (root `pnpm run ci`, per-project `pnpm run lint|test|build|dev`).

- **Node version / network caveat:** `engine-strict=true` (`.npmrc`) requires Node **exactly `26.1.0`**. The sandbox network policy (`.cursor/sandbox.json`) blocks `nodejs.org`, so Node 26.1.0 was installed into nvm (`~/.nvm/versions/node/v26.1.0`) by extracting the npm package `node-linux-x64@26.1.0` (npm registry and GitHub are reachable; `nvm install` is not). `~/.bashrc` prepends that nvm bin to `PATH` so Node 26 wins over `/exec-daemon`'s Node 22. If `node --version` ever shows v22, run `nvm use` (reads `.nvmrc`) or `export PATH="$HOME/.nvm/versions/node/v26.1.0/bin:$PATH"`. `pnpm` (11.0.8) comes from Corepack via the `packageManager` field.
- **Main UI / dev server:** the documentation site is the primary browser app: `cd projects/site && pnpm run dev` serves at **`http://localhost:8082/elements/`** (note the `/elements/` base path; the bare `/` redirects). `pnpm run dev` rebuilds all sibling library deps first via Wireit; use `pnpm run dev:fast` to skip the dep rebuild when libraries are already built. The site has no standalone "playground"; every component page renders live interactive examples.
- **Wireit builds deps on demand:** the first `lint`/`test`/`build` in a library (e.g. `projects/core`) transparently builds sibling packages (`forms`, `themes`, `styles`, `monaco`, `metadata`, `lint`, …), taking ~1 minute; subsequent runs are cached in `.wireit/`.
- **Tests are browser-based:** unit/axe/visual tests run in Chromium via vitest + Playwright (headless shell + ffmpeg cached in `~/.cache/ms-playwright`). Run a single suite with `pnpm run test -- src/<name>/<name>.test.ts` from the project dir.
- **Git LFS smudge noise:** after an LFS checkout/build, `git status` may list many `projects/*/.visual/*.png` and `projects/internals/metadata/static/*.json` as modified even though content is unchanged. These are not real edits — do not commit them; run `git checkout -- projects/` to clear them.
