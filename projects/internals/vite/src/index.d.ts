...
export interface WebGPUAdapterInfo {
  ...
  description?: string;
  device?: string;
  isFallbackAdapter?: boolean | 'unavailable';
  vendor?: string;
}
...
export interface WebGPUObserverSnapshot {
  ...
  epoch: number;
  externalImageCopies: Array<{ deviceId: number; epoch: number }>;
  renderPipelines: Array<{
    deviceId: number;
    epoch: number;
    fragmentEntryPoint: string | null;
    shaderModuleId: number | null;
  ...
  shaderModules: Array<{ code: string; deviceId: number; epoch: number; id: number }>;
