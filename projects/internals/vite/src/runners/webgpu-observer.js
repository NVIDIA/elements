...
(() => {
  ...
  globalThis.__webgpuTestObserver = {
    ...
    snapshot() {
      return {
        ...
        epoch,
        externalImageCopies: externalImageCopies.map(value => ({ ...value })),
        renderPipelines: renderPipelines.map(value => ({ ...value })),
        renderPasses: renderPasses.map(value => ({ ...value })),
        submits: submits.map(value => ({ ...value })),
        shaderModules: shaderModules.map(value => ({ ...value })),
  ...
  function observeDevice(device) {
      ...
      original =>
        function (...args) {
          ...
          for (const info of buffers) {
            if (info.deviceId === activeDeviceId) markResourceDestroyed(info, 'buffer');
          }
          for (const info of textures) {
            if (info.deviceId === activeDeviceId) markResourceDestroyed(info, 'texture');
          }
    ...
    wrapPrototypeMethod(
      ...
      original =>
        function (descriptor) {
          const shader = Reflect.apply(original, this, [descriptor]);
          const activeDeviceId = deviceIds.get(this) ?? deviceId;
          const id = nextResourceId++;
          shaderModuleIds.set(shader, id);
          shaderModules.push({ code: String(descriptor.code ?? ''), deviceId: activeDeviceId, epoch, id });
          return shader;
        }
    );
    wrapPrototypeMethod(
      ...
      original =>
        function (descriptor) {
          const pipeline = Reflect.apply(original, this, [descriptor]);
          const activeDeviceId = deviceIds.get(this) ?? deviceId;
          recordRenderPipeline(descriptor, activeDeviceId);
          return pipeline;
        }
    );
    wrapPrototypeMethod(
      ...
      original =>
        function (descriptor) {
          const pipelinePromise = Reflect.apply(original, this, [descriptor]);
          const activeDeviceId = deviceIds.get(this) ?? deviceId;
  ...
  function observeQueue(queue, deviceId) {
    ...
    wrapPrototypeMethod(
      ...
      original =>
        function (...args) {
          externalImageCopies.push({ deviceId: queueDeviceIds.get(this) ?? deviceId, epoch });
          return Reflect.apply(original, this, args);
        }
    );
    wrapPrototypeMethod(
    ...
    wrapPrototypeMethod(
      ...
      original =>
        function (...args) {
          ...
          markResourceDestroyed(info, 'buffer');
          return Reflect.apply(original, this, args);
        }
    );
  }
    ...
    wrapPrototypeMethod(
      ...
      original =>
        function (...args) {
          ...
          markResourceDestroyed(info, 'texture');
          return Reflect.apply(original, this, args);
        }
    );
  }

  function markResourceDestroyed(info, kind) {
    if (!info || info.destroyed) return;
    info.destroyed = true;
    info.destroyedEpoch = epoch;
    destroys.push({ epoch, id: info.id, kind });
  }

  function recordRenderPipeline(descriptor, deviceId) {
    renderPipelines.push({
      deviceId,
      epoch,
      fragmentEntryPoint: descriptor.fragment?.entryPoint ?? null,
      shaderModuleId: shaderModuleIds.get(descriptor.vertex?.module) ?? null,
