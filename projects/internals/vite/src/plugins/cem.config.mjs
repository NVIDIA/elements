...
// Vue normalizes Boolean attribute aliases to their linked properties. Keep the camel-cased property binding without
// duplicating it as a kebab-cased Boolean prop. Preserve attribute-only Boolean APIs.
export function omitVueBooleanAttributeAliases(declaration) {
  const memberNames = new Set(declaration.members?.map(member => member.name));

  declaration.attributes = declaration.attributes?.filter(
    attribute => attribute.type?.text !== 'boolean' || !attribute.fieldName || !memberNames.has(attribute.fieldName)
  );
}
...
const vueTypesManifestCache = new WeakMap();

function getVueTypesManifest(customElementsManifest) {
  if (!vueTypesManifestCache.has(customElementsManifest)) {
    const vueTypesManifest = structuredClone(getStandaloneTypesManifest(customElementsManifest));

    vueTypesManifest.modules
      .flatMap(module => module.declarations ?? [])
      .filter(declaration => declaration.tagName)
      .forEach(omitVueBooleanAttributeAliases);

    vueTypesManifestCache.set(customElementsManifest, vueTypesManifest);
  }

  return vueTypesManifestCache.get(customElementsManifest);
}

function withStandaloneTypes(plugin, getManifest = getStandaloneTypesManifest) {
  return {
    ...plugin,
    packageLinkPhase({ customElementsManifest }) {
      plugin.packageLinkPhase({ customElementsManifest: getManifest(customElementsManifest) });
    }
  };
}
