...
export function validateTemplate(
...
): { result: string; status: 'success' | 'error'; message?: string } {
  ...
  if (!mergedConfig.allowGlobalElements) {
    if (result.includes('<html') || result.includes('<body') || result.includes('<head')) {
      status = 'error';
      message = 'Do not include `html`, `body` tags or custom CSS.';
    }
  }
