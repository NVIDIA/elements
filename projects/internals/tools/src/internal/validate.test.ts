...
describe('validateTemplate', () => {
  ...
  it('should sanitize a template', () => {
    ...
    const result = validateTemplate(template, elements, { allowVulnerableTags: false });
    expect(result).toBe('<p>hello</p>');
  });

  it('should not sanitize a template', () => {
    const template = '<p>hello</p><script>alert("!")</script>';
    const result = validateTemplate(template, elements, { allowVulnerableTags: true });
  ...
  it('should not allow global elements', () => {
    ...
    expect(validateTemplate(template, elements)).toBe('');
  });

  it('should allow global elements', () => {
    const template = '<body></body>';
    const result = validateTemplate(template, elements, { allowGlobalElements: true });
    expect(result).toBe('<body></body>');
  });

  it('should remove style attributes', () => {
    const template = '<nve-badge style="--color: red">hello there</nve-badge>';
    const result = validateTemplate(template, elements, { allowStyles: false });
    expect(result).toBe('<nve-badge>hello there</nve-badge>');
  });

  it('should allow style attributes by default', () => {
    const template = '<nve-badge style="--color: red">hello there</nve-badge>';
    const result = validateTemplate(template, elements, { allowStyles: true });
