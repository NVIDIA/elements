...
describe(Gauge.metadata.tag, () => {
  ...
  /** Settles the progress animation so assertions read the value instead of an in-flight frame. */
  const disableAnimation = (gauge: Gauge) => gauge.style.setProperty('--_animation-duration', '0s');
  ...
  it('should animate progress and the end dot toward a new value', async () => {
    element.style.setProperty('--_animation-duration', '250ms');
    element.value = 25;
    await elementIsStable(element);

    const dot = element.shadowRoot.querySelector('.fill-dot-end') as SVGCircleElement;
    const [start] = element.getAnimations();
    start.finish();
    await start.finished;
    await elementIsStable(element);

    expect(renderedProgress()).toBeCloseTo(0.25, 2);
    expect(dot.style.getPropertyValue('--_dot-angle')).toBe('202.5deg');

    element.value = 75;
    await elementIsStable(element);

    // the fill holds the previous value until the animation advances it
    expect(renderedProgress()).toBeCloseTo(0.25, 2);

    const [animation] = element.getAnimations();
    expect(animation.effect.getTiming().duration).toBe(250);
    expect(animation.effect.getTiming().easing).toBe('ease-in-out');

    animation.currentTime = 125;
    await new Promise(resolve => requestAnimationFrame(resolve));
    await elementIsStable(element);

    const midProgress = renderedProgress();
    expect(midProgress).toBeGreaterThan(0.25);
    expect(midProgress).toBeLessThan(0.75);
    expect(parseFloat(dot.style.getPropertyValue('--_dot-angle'))).toBeCloseTo(135 + 270 * midProgress, 1);
  ...
  it('should keep the fill painted until it animates back to zero', async () => {
    element.style.setProperty('--_animation-duration', '250ms');
    element.value = 50;
    await elementIsStable(element);

    const [fillIn] = element.getAnimations();
    fillIn.finish();
    await fillIn.finished;
    await elementIsStable(element);
  ...
  it('should settle progress when removed and reinserted while animating', async () => {
    element.style.setProperty('--_animation-duration', '250ms');
    element.value = 100;
    await elementIsStable(element);
    ...
    expect(renderedProgress()).toBeGreaterThan(0);
    expect(renderedProgress()).toBeLessThan(1);
  ...
  it('should not animate progress on the first render', async () => {
    removeFixture(fixture);
    fixture = await createFixture(html`
      <nve-gauge value="80"></nve-gauge>
    `);
    element = fixture.querySelector(Gauge.metadata.tag);
    await elementIsStable(element);

    expect(element.getAnimations()).toHaveLength(0);
    expect(renderedProgress()).toBeCloseTo(0.8, 2);
  });

  it('should settle immediately when the animation duration is zero', async () => {
    element.value = 60;
    await elementIsStable(element);

    expect(element.getAnimations()).toHaveLength(0);
    expect(renderedProgress()).toBeCloseTo(0.6, 2);
  });
