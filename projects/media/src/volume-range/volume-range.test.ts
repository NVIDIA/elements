...
describe(MediaVolumeRange.metadata.tag, () => {
  ...
  it('should sync disabled state from the target controller', async () => {
    ...
    expect(new FormData(form).get('volume')).toBe('0.4');

    setMediaState(target, { volume: Number.POSITIVE_INFINITY });
    await elementIsStable(range);
    ...
    const events = vi.fn();
    range.addEventListener('input', events);
    ...
    expect(input.disabled).toBe(false);
    expect(range.value).toBe(1);
    expect(new FormData(form).get('volume')).toBe('1');
  });
  ...
  it('should dispatch the host change event for user input', async () => {
    const inputEvent = untilEvent(range, 'input');
  ...
  it('should handle zero-length range progress', async () => {
    range.min = 1;
    range.max = 1;
    await elementIsStable(range);

    expect(range.style.getPropertyValue('--track-progress')).toBe('0');
  });
