...
export class MediaStateController<T extends MediaStateHost> implements ReactiveController {
  ...
  #syncCommandTarget(stateSyncTiming: StateSyncTiming = 'immediate') {
    const commandTargetInput = this.host.commandForElement ?? this.host.getAttribute('commandfor');
    if (commandTargetInput === this.#commandTargetInput) {
      return;
    }

    const commandTarget = getCommandTarget(this.host);
    if (commandTarget) {
      this.#commandTargetInput = commandTargetInput;
    }
