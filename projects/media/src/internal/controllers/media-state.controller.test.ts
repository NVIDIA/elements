...
describe(MediaStateController.name, () => {
  ...
  it('should skip target lookup when command target input is unchanged', async () => {
    target.id = 'target';
    element.setAttribute('commandfor', target.id);
    element.commandForElement = null;
    await elementIsStable(element);
    const getRootNode = vi.spyOn(element, 'getRootNode');
  ...
  it('should retry unresolved command target input', async () => {
    element.setAttribute('commandfor', 'late-target');
    element.commandForElement = null;
    await elementIsStable(element);
    expect(element.mediaState).toBeNull();
  ...
  it('should retarget, disconnect, and reconnect through the host lifecycle', async () => {
