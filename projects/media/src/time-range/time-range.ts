...
import { isMediaTimeSpans, type MediaState, type MediaTimeSpan } from '../internal/media-state.js';
...
export class MediaTimeRange extends SliderFormControlMixin(LitElement) {
  ...
  // eslint-disable-next-line local/primitive-property -- Disjoint buffered time spans map directly to native TimeRanges.
  @property({ type: Array, attribute: 'buffered-ranges' })
  bufferedRanges: readonly MediaTimeSpan[] = [];
  ...
  @state() protected mediaBufferedRanges: readonly MediaTimeSpan[] | null = null;
  ...
  #renderBufferedRanges() {
    const bufferedRanges = this.mediaBufferedRanges ?? this.bufferedRanges;
    return isMediaTimeSpans(bufferedRanges) ? bufferedRanges.map(span => this.#renderBufferedRange(span)) : nothing;
  }

  #renderBufferedRange({ start, end }: MediaTimeSpan) {
    const range = this.max - this.min;
    if (!Number.isFinite(range) || range <= 0) {
      return nothing;
    }

    const rangeStart = clamp((start - this.min) / range, 0, 1);
    const rangeEnd = clamp((end - this.min) / range, 0, 1);
    ...
    return html`
      <span
        ...
        aria-hidden="true"
        style=${styleMap({
          '--buffered-range-start': `${rangeStart}`,
          '--buffered-range-size': `${rangeEnd - rangeStart}`
        })}
      ></span>
    `;
  }
  ...
  #handleMediaState = (mediaState: MediaState | null) => {
    this.mediaBufferedRanges = mediaState?.buffered ?? null;
    if (!mediaState) {
      return;
    }
