...
describe(MediaPlaybackRateSelect.metadata.tag, () => {
  ...
  it('should warn when value does not match an available rate', async () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      'nve-media-playback-rate-select: value "1.25" does not match any available rates.'
    );
  ...
  it('should ignore user changes while disabled or readonly', async () => {
  ...
  it('should ignore invalid target playback rates', async () => {
    setMediaState(target, { playbackRate: 0 });
    await elementIsStable(select);
    expect(select.value).toBe('1');

    setMediaState(target, { playbackRate: Number.NaN });
    await elementIsStable(select);
    expect(select.value).toBe('1');
  });

  it('should normalize invalid and missing rate lists', async () => {
    select.rates = [0, -1, Number.NaN, 'invalid'];
    await elementIsStable(select);
    expect(select.options.map(option => option.value)).toEqual(['1']);

    select.rates = null as unknown as Array<number | string>;
    await elementIsStable(select);
    expect(select.options.map(option => option.value)).toEqual(['0.5', '1', '1.5', '2']);
  });
