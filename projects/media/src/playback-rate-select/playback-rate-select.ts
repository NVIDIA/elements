...
import { I18nController, LogService, useStyles } from '@nvidia-elements/core/internal';
...
export class MediaPlaybackRateSelect extends SelectFormControlMixin(LitElement) {
  ...
  override updated(changedProperties: PropertyValues<this>) {
    ...
    if (changedProperties.has('value') || changedProperties.has('rates')) {
      this.#warnIfValueIsUnavailable();
    }
  }
  ...
  #warnIfValueIsUnavailable() {
    const value = this.getAttribute('value') ?? this.value;
    if (!this.options.map(option => option.value).includes(value)) {
      LogService.warn(`${this.localName}: value "${value}" does not match any available rates.`);
    }
  }
