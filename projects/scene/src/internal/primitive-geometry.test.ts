...
describe('primitive geometry', () => {
  ...
  it('should convert arrow endpoints to origin, shortest-arc orientation, and uniform length', () => {
    expect(markerFromPointPair([1, 2, 3], [1, 2, 5], [1, 0, 0, 1])).toEqual({
      position: [1, 2, 3],
      orientation: [0, 0, 0, 1],
      scale: [2, 2, 2],
      color: [1, 0, 0, 1]
    });
    expect(markerFromPointPair([0, 0, 0], [0, 0, -2], undefined).orientation).toEqual([1, 0, 0, 0]);
    expect(markerFromPointPair([0, 0, 0], [2, 0, 0], undefined).orientation).toEqual([
      0,
      expect.closeTo(Math.SQRT1_2, 5),
      0,
      expect.closeTo(Math.SQRT1_2, 5)
    ]);
    expect(() => markerFromPointPair([1, 1, 1], [1, 1, 1], undefined)).toThrow(RangeError);
  });
});
