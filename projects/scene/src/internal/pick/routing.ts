...
export type ScenePickDriver = (request: ScenePickRequest) => Promise<ScenePickResult | null>;
...
export function requestScenePick(options: {
  ...
  return driver ? driver(request) : Promise.resolve(null);
}
...
function getRendererPickDriver(renderer: unknown): ScenePickDriver | undefined {
  ...
  return request => Promise.resolve(Reflect.apply(pick, renderer, [request])).then(validateRendererPickResult);
}
