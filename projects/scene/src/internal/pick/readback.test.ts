...
describe(PickReadback.name, () => {
  it('should copy one aligned pixel, retain its target snapshot, and map without stalling submission', async () => {
    ...
    const reused = readback.copy({
      encoder,
      frame: {
        decodeTarget: id => (id === 1 ? { value: 'second' } : undefined),
    ...
    await vi.waitFor(() => expect(resolveMap).toBeTypeOf('function'));
    resolveRequired(resolveMap);
    await expect(reused).resolves.toMatchObject({ target: { value: 'second' } });
  ...
  it('should clean up and return misses for unavailable mapping, unknown IDs, and invalid depth', async () => {
    ...
    for (const { id, depth } of cases) {
      ...
      const caseReadback = new PickReadback<{ value: string }>(device);
      const pending = caseReadback.copy({
        encoder,
        frame: {
          decodeTarget: targetId => (targetId === 1 ? { value: 'hit' } : undefined),
          inverseViewProjection: identityMat4()
  ...
  it('releases a buffer when recording a texture copy throws', () => {
    const buffer: SceneGPUBuffer = { destroy: vi.fn() };
    const readback = new PickReadback<{ value: string }>({
      createBuffer: () => buffer,
      createCommandEncoder: () => ({ beginRenderPass: () => ({ end: () => undefined }), finish: () => ({}) }),
      destroy: () => undefined,
...
function resolveRequired(callback: (() => void) | undefined): void {
  if (!callback) throw new Error('Expected pending buffer mapping.');
  callback();
}
