...
import type { PickCompletion, PickRequest, PickRequestKind, ResolvedPickHit } from './types.js';

interface PickCoordinatorOptions<T = ResolvedPickHit> {
  readonly onComplete?: (completion: PickCompletion<T>) => void;
  readonly onStaleHover?: (request: PickRequest) => void;
}

interface PickRequestHandle<T = ResolvedPickHit> {
 ...
 */
export class PickCoordinator<T = ResolvedPickHit> {
  #nextSequence = 0;
  #nextOrdered = 1;
  ...
  /** Invalidates hover work without disturbing ordered pointer requests. */
  cancelHover(): void {
    const active = this.#activeHover;
    const queued = this.#queuedHover;
    this.#activeHover = undefined;
    this.#queuedHover = undefined;
    if (active) this.#settleStaleHover(active);
    if (queued) this.#settleStaleHover(queued);
  }
  ...
  #settleActiveHover(pending: PendingHover<T>, outcome: { readonly error?: unknown; readonly hit?: T | null }): void {
    if (this.#activeHover !== pending) return;
    try {
      if (this.#queuedHover) {
