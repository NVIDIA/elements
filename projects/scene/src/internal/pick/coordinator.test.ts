...
describe(PickCoordinator.name, () => {
  ...
  it('bounds a hover burst to one active resolver', async () => {
    let active = 0;
    let maximumActive = 0;
    const resolvers = new Map<number, (value: string) => void>();
    const coordinator = new PickCoordinator<string>();
    ...
    const burst = [hover()];
    await Promise.resolve();
    for (let index = 1; index < 100; index += 1) burst.push(hover());
    resolvers.get(1)?.('old');
    await vi.waitFor(() => expect(resolvers.has(100)).toBe(true));
    resolvers.get(100)?.('latest');

    await expect(Promise.all(burst)).resolves.toEqual([...Array.from({ length: 99 }, () => null), 'latest']);
    expect(maximumActive).toBe(1);
    expect([...resolvers.keys()]).toEqual([1, 100]);
  });

  it('cancels active and queued hover requests without canceling ordered requests', async () => {
    const resolvers = new Map<number, (value: string) => void>();
    const completed: number[] = [];
    const coordinator = new PickCoordinator<string>({ onComplete: ({ request }) => completed.push(request.sequence) });
    const hover = coordinator.request(
      'hover',
      ({ sequence }) => new Promise(resolve => resolvers.set(sequence, resolve))
    );
    await Promise.resolve();
    const queued = coordinator.request('hover', () => 'queued');
    const click = coordinator.request('click', () => 'click');

    coordinator.cancelHover();
    resolvers.get(1)?.('late');

    await expect(hover.result).resolves.toBeNull();
    await expect(queued.result).resolves.toBeNull();
    await expect(click.result).resolves.toBe('click');
    expect(completed).toEqual([3]);
  });
