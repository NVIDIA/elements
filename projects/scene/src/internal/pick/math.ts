...
import type { PickTableEntry, ResolvedPickHit } from './types.js';
...
export function reconstructWorldPosition(options: ReconstructWorldPositionOptions): Vec3 | null {
  ...
  const [worldX, worldY, worldZ, worldW] = multiplyMat4Vec4(matrix, [ndcX, ndcY, options.depth, 1]);
...
export function createPickHit(entry: PickTableEntry, worldPosition: Readonly<Vec3>): ResolvedPickHit {
