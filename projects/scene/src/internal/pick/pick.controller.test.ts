...
import { GestureController, type Gesture, type GestureInput } from '@nvidia-elements/core/internal';
import { PickController } from './pick.controller.js';
import type { SceneClick, ScenePickDriver, ScenePickHit, ScenePickResult } from './routing.js';
...
describe(PickController.name, () => {
  ...
  it('routes an accepted blocked pointer through the same typed driver boundary', async () => {
    ...
    const picking = new PickController({
      ...
      host,
      updateClickSource: () => document.createElement('span')
    });
  ...
  it('routes canvas pointer input only while its reactive host is connected', async () => {
    ...
    const dispatchClick = (): void => {
      canvas.dispatchEvent(new PointerEvent('click', { clientX: 10, clientY: 10 }));
    };
  ...
  it('routes unclaimed canvas input and skips a pointer claimed during capture', async () => {
    const driver = vi.fn(() => Promise.resolve(createResult(layer)));
    const { canvas, layer } = createInteractiveController(driver);
    const down = vi.fn();
    const enter = vi.fn();
    layer.addEventListener('pointerdown', down);
    layer.addEventListener('nve-scene-pointerenter', enter);

    const claim = (event: PointerEvent): void => event.stopPropagation();
    canvas.addEventListener('pointerdown', claim, true);
    canvas.dispatchEvent(new PointerEvent('pointerdown', { bubbles: true, clientX: 10, clientY: 10 }));
    await Promise.resolve();
    expect(driver).not.toHaveBeenCalled();

    canvas.removeEventListener('pointerdown', claim, true);
    canvas.dispatchEvent(new PointerEvent('pointerdown', { bubbles: true, clientX: 10, clientY: 10 }));
  ...
  it('routes claimed camera pointer starts and releases to a scene layer once', async () => {
    const driver = vi.fn(() => Promise.resolve(createResult(layer)));
    const { canvas, host, layer } = createInteractiveController(driver);
    const events: string[] = [];
    const drags: number[] = [];
    layer.addEventListener('pointerdown', () => events.push('down'));
    layer.addEventListener('pointerup', () => events.push('up'));
    host.addEventListener('nve-gesture', (event: Event) => {
      const gesture = (event as CustomEvent<Gesture<undefined>>).detail;
      if (gesture.kind === 'drag') drags.push(gesture.movementX);
    });
    const gesture = new GestureController(host, {});
    gesture.hostConnected();
    gesture.target = canvas;
    host.addEventListener('nve-gesture-input', (event: Event) => {
      const input = (event as CustomEvent<GestureInput>).detail;
      if (input.kind === 'pointerdown') input.claim({ kind: input.event.button === 2 ? 'pan' : 'drag' });
    });
    ...
    canvas.dispatchEvent(
      new PointerEvent('pointermove', { bubbles: true, buttons: 1, pointerId: 7, clientX: 15, clientY: 10 })
    );
    canvas.dispatchEvent(
      new PointerEvent('pointerup', { bubbles: true, button: 0, pointerId: 7, clientX: 10, clientY: 10 })
    );

    await vi.waitFor(() => expect(events).toEqual(['down', 'up']));
    expect(drags).toEqual([5]);
    expect(driver).toHaveBeenCalledTimes(2);

    canvas.dispatchEvent(
      new PointerEvent('pointerdown', { bubbles: true, button: 2, pointerId: 8, clientX: 10, clientY: 10 })
    );
    canvas.dispatchEvent(
      new PointerEvent('pointerup', { bubbles: true, button: 2, pointerId: 8, clientX: 10, clientY: 10 })
    );
    await Promise.resolve();
    expect(driver).toHaveBeenCalledTimes(2);
  });
  ...
  it('updates and exposes one click source before dispatching the routed event', async () => {
    const source = document.createElement('span');
    const order: string[] = [];
    const updateClickSource = vi.fn(() => {
      order.push('source');
      return source;
    });
    const { host, layer, picking } = createInteractiveController(
      () => Promise.resolve({ ...createResult(layer), featureId: 0 }),
      () => true,
      updateClickSource
    );
    let received: CustomEvent<SceneClick> | undefined;
    host.addEventListener('click', () => order.push('click'));
    host.addEventListener('nve-scene-click', event => {
      order.push('scene-click');
      received = event;
    });
    ...
    expect(updateClickSource).toHaveBeenCalledOnce();
    expect(updateClickSource).toHaveBeenCalledWith(expect.objectContaining({ element: layer }));
    expect(received?.target).toBe(layer);
    expect(received?.detail.featureId).toBe(0);
    expect(received?.detail.source).toBe(source);
    expect(Object.isFrozen(received?.detail)).toBe(true);
    expect(order).toEqual(['source', 'click', 'scene-click']);
  });

  it('deactivates the click source when automatic picking misses', async () => {
    const updateClickSource = vi.fn(() => document.createElement('span'));
    const { picking } = createInteractiveController(
    ...
    await vi.waitFor(() => expect(updateClickSource).toHaveBeenCalledWith(null));
  });
...
function createInteractiveController(
  ...
  updateClickSource: (hit: ScenePickHit | null) => HTMLElement = () => document.createElement('span')
): {
  readonly canvas: HTMLCanvasElement;
  readonly host: PickControllerTestHost;
