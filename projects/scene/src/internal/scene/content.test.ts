...
describe(SceneContent.name, () => {
  ...
  it('falls back to validated stream configuration values when a malformed layer bypasses its accessor', () => {
    const host = document.createElement('nve-scene');
    const points = document.createElement('nve-scene-points');
    const lines = document.createElement('nve-scene-lines');
    Object.defineProperty(points, 'sizeUnit', { configurable: true, value: 'invalid' });
    Object.defineProperty(lines, 'topology', { configurable: true, value: 'invalid' });
    Object.defineProperty(lines, 'widthUnit', { configurable: true, value: 'invalid' });
    host.append(points, lines);
    const content = new SceneContent(host);
    ...
    expect(content.compileRenderItems()).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ sizeUnit: 'pixel', type: 'point' }),
        expect.objectContaining({ topology: 'strip', type: 'line', widthUnit: 'world' })
      ])
    );
  });
