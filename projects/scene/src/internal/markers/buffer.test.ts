...
describe('marker buffer', () => {
  it('should allocate fixed record capacity while tracking added records', () => {
    ...
    expect(markers.mutableBytes).toHaveLength(2 * MARKER.stride);
    expect(readMarker(markers.mutableBytes, 1)).toMatchObject({
      orientation: [0, 0, 0, 1],
      scale: [1, 1, 1]
    });
  ...
  it('should write initial fields and supported CSS colors into the canonical layout', () => {
    ...
    expect(readMarker(markers.mutableBytes, 0)).toEqual({
      position: [1, 2, 3],
      orientation: [0, 0, 0, 1],
      scale: [2, 3, 4],
      color: [0, 1, 1, 1],
  ...
  it('should mutate one record through stable transform and color handles', () => {
    ...
    expect(readMarker(markers.mutableBytes, 0)).toMatchObject({ position: [4, 2, 3], scale: [2, 3, 4] });
    expect(readMarker(markers.mutableBytes, 1).position).toEqual([9, 9, 9]);
  });
  ...
  it('should write without allocating handles and lazily return stable handles', () => {
    ...
    expect(readMarker(markers.mutableBytes, 1).position).toEqual([4, 5, 6]);
