...
import type { ExternalLineVertexSource } from '../../record-sources.js';
...
class LineVertexRecord implements LineVertex {
  ...
  constructor(view: DataView, index: number, notifyMutation: () => void) {
    ...
    this.position = new MutableVector3View(view, this.#recordOffset + POSITION_OFFSET, { notifyMutation });
    this.normal = new MutableVector3View(view, this.#recordOffset + NORMAL_OFFSET, {
      notifyMutation,
      validate: assertNonzeroNormal
    });
  }
  ...
  setStyle(style: LineVertexStyle): this {
    ...
    this.#view.setFloat32(this.#recordOffset + NORMAL_OFFSET, next.normal[0], true);
    this.#view.setFloat32(this.#recordOffset + NORMAL_OFFSET + 4, next.normal[1], true);
    this.#view.setFloat32(this.#recordOffset + NORMAL_OFFSET + 8, next.normal[2], true);
