...
export function createMeshPipelines(device: SceneGPURenderPipelineDevice, format: string): MeshPipelines {
...
/** Composes mesh color and ID/depth shaders from one vertex implementation. */
...
@vertex fn vertexMain(input: VertexInput) -> VertexOutput {
  ...
  output.uv = input.uv;
  ${pick ? 'output.id = scene.pickId + input.instanceIndex;' : ''}
  return output;
}
${fragment}
`;
...
const MESH_SHADER = createMeshShader({ pass: 'color' });
