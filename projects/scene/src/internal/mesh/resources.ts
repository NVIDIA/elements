...
interface UploadMeshGeometryBufferOptions {
  readonly buffer: SceneGPUBuffer;
  readonly device: MeshBufferDevice;
...
export function createGeneratedMeshGeometryResources(
  ...
  const buffers: SceneGPUBuffer[] = [];
  const track = (buffer: SceneGPUBuffer): SceneGPUBuffer => {
    buffers.push(buffer);
    return buffer;
  };
  try {
    const colors = track(createGeneratedVertexBuffer(device, vertexCount * 4));
    const index = track(createIndexBuffer(device, indices));
    const normals = track(createGeneratedVertexBuffer(device, vertexCount * 3));
    const positions = track(createGeneratedVertexBuffer(device, vertexCount * 3));
    const uvs = track(
      device.createBuffer({ size: vertexCount * 2 * Float32Array.BYTES_PER_ELEMENT, usage: BUFFER_VERTEX })
    );
...
export function uploadMeshGeometryBuffer(options: UploadMeshGeometryBufferOptions): void {
  const { buffer, device, values } = options;
  const range = options.range ?? { offset: 0, size: values.byteLength };
  const data =
    range.offset === 0 && range.size === values.byteLength
      ? values
      : new Uint8Array(values.buffer, values.byteOffset + range.offset, range.size);
  device.queue.writeBuffer(buffer, range.offset, data);
}
