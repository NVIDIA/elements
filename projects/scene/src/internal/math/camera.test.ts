...
describe('camera math', () => {
  ...
  it('moves the orbit camera toward world up when the pointer moves down', () => {
    const state = copyOrbitCameraState(DEFAULT_ORBIT_CAMERA_STATE);
    const down = applyOrbitDrag(state, { movementX: 0, movementY: 10 });
    const up = applyOrbitDrag(state, { movementX: 0, movementY: -10 });

    expect(cameraEye(down)[2]).toBeGreaterThan(cameraEye(state)[2]);
    expect(cameraEye(up)[2]).toBeLessThan(cameraEye(state)[2]);
  });
