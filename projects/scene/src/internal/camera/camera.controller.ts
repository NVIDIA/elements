...
import { SceneCamera, sceneCameraController, setCameraRuntimeProperty } from '../../camera/camera.js';
import { getCameraInputRevision } from './input.js';
import {
  GestureController,
...
import { getFrameWorldMatrixPrecise, getNamedSceneFrame, isFrameChainValid } from '../frame/state.js';
import {
  applyOrbitDrag,
  applyOrbitKey,
  applyOrbitWheel,
...
import type { Quaternion, ScenePose } from '../types.js';
...
interface CameraBehaviorSnapshot {
  readonly behavior: SceneCamera;
  readonly contribution: CameraBehaviorContribution | null;
...
export class CameraController implements ReactiveController {
  ...
  constructor(options: { readonly host: CameraHost; readonly requestRender: () => void }) {
    ...
    this.#gestureController = new GestureController(options.host, {
      beginPinch: () => {
        this.resolve();
        return this.#getInteractiveOrbit() ? { context: this.#orbitState.offset.distance } : undefined;
      },
      shouldPreventContextMenu: () => {
        this.resolve();
        return Boolean(this.#getInteractiveOrbit() || this.#getInteractiveTop());
      },
  ...
  trackChanges(): boolean {
    const previous = this.#behaviorSnapshot;
  ...
  resolve(): void {
    ...
    this.#gestureController.target =
      this.#getInteractiveOrbit() || this.#getInteractiveTop() ? this.#canvas : undefined;
  }
  ...
  dispatchPendingChange(): void {
    ...
    this.#host.dispatchEvent(
      new CustomEvent<SceneCameraChangeDetail>('nve-scene-camera-change', {
        bubbles: true,
        cancelable: false,
        composed: true,
  ...
  #resolveBehaviorContributions(snapshot: readonly CameraBehaviorSnapshot[]): ResolvedCameraBehavior[] {
    const configured = snapshot
      .map(({ behavior, contribution }) => ({ behavior, contribution }))
      .filter(
  ...
  #captureBehaviorSnapshot(): readonly CameraBehaviorSnapshot[] {
    const previous = this.#behaviorSnapshot;
    if (previous && this.#isBehaviorSnapshotCurrent(previous)) return previous;
    const snapshot = getSceneCameras(this.#host).map(behavior => ({
      behavior,
      contribution: sceneCameraController.getContribution(behavior),
      revision: getCameraInputRevision(behavior)
    }));
    return this.#isBehaviorSnapshotCurrent(snapshot) ? snapshot : this.#captureBehaviorSnapshotAfterSynchronousWrite();
  }

  #captureBehaviorSnapshotAfterSynchronousWrite(): readonly CameraBehaviorSnapshot[] {
    return getSceneCameras(this.#host).map(behavior => ({
      behavior,
      contribution: sceneCameraController.getContribution(behavior),
      revision: getCameraInputRevision(behavior)
    }));
  ...
  #isBehaviorSnapshotCurrent(snapshot: readonly CameraBehaviorSnapshot[]): boolean {
    let index = 0;
    for (const element of this.#host.querySelectorAll('nve-scene-camera')) {
      if (!(element instanceof SceneCamera) || element.closest('nve-scene') !== this.#host) continue;
      const entry = snapshot[index];
      if (!entry || !cameraSnapshotEntryMatchesCurrent(entry, element)) return false;
      index += 1;
    }
  ...
  #toOrbitCameraContribution(contribution: Extract<CameraBehaviorContribution, { kind: 'orbit' }>): CameraContribution {
    ...
    return {
      ...
      patch: {
        ...(target ? { target } : {}),
        offset: { distance: contribution.distance, polarAngle: contribution.polarAngle, azimuth: contribution.azimuth },
  ...
  #toTopCameraContribution(contribution: Extract<CameraBehaviorContribution, { kind: 'top' }>): CameraContribution {
    return {
      ...
      patch: {
        ...
        offset: { distance: contribution.altitude, polarAngle: 0, azimuth: 0 },
        projection: contribution.projection
      }
    };
  }
  ...
  #toFollowCameraContribution(
    ...
    const matrix = getFrameWorldMatrixPrecise(frame);
    return {
      fields: contribution.fields,
      patch: {
        target: {
          position: [matrix[12]!, matrix[13]!, matrix[14]!],
          ...(contribution.followMode === 'pose' ? { heading: Math.atan2(matrix[1]!, matrix[0]!) } : {})
        }
      }
    };
  }

  #resolvePoseCameraState(contribution: Extract<CameraBehaviorContribution, { kind: 'pose' }>): SceneCameraState {
  ...
  #composeFramePose(frameName: string, localPose: ScenePose): ScenePose {
    ...
    return {
      ...
      orientation: multiplyQuaternions(frameOrientation(matrix), localPose.orientation)
    };
  }
  ...
  #handleGestureInput = (event: CustomEvent<GestureInput>): void => {
    ...
    if (input.kind === 'pointerend') {
      if (input.event.button === 2) input.event.preventDefault();
      return;
    }
    this.resolve();
    if (input.kind === 'wheel') {
      this.#handleWheelInput(input);
    } else {
      this.#handlePointerDownInput(input);
    }
  };

  #handleWheelInput(input: WheelGestureInput): void {
    const orbit = this.#getInteractiveOrbit();
    if (orbit && input.claim()) this.#setUserState(applyOrbitWheel(this.#orbitState, input.deltaY, orbit), 'wheel');
  }

  #handlePointerDownInput(input: PointerDownInput): void {
    const orbit = this.#getInteractiveOrbit();
    const interactive = Boolean(orbit || this.#getInteractiveTop());
    const pointer = input.event;
    if (pointer.button === 2 && interactive) {
      if (input.claim({ kind: 'pan' })) pointer.preventDefault();
    } else if (orbit && (pointer.button === 0 || pointer.pointerType === 'touch')) {
      input.claim({ kind: 'drag', pinchCandidate: pointer.pointerType === 'touch' });
    }
  }
  ...
  #handleGesture(gesture: Gesture<number>): void {
    if (gesture.kind === 'drag') return this.#handleDrag(gesture);
    if (gesture.kind === 'pan') return this.#handlePan(gesture);
    if (gesture.kind === 'pinch') return this.#handlePinch(gesture);
  ...
  #handleDrag(gesture: Extract<Gesture<number>, { kind: 'drag' }>): void {
    const orbit = this.#getInteractiveOrbit();
    if (!orbit) return;
    this.#setUserState(applyOrbitDrag(this.#orbitState, gesture, orbit), pointerCameraSource(gesture.event));
  }
  ...
  #handlePinch(gesture: Extract<Gesture<number>, { kind: 'pinch' }>): void {
    const orbit = this.#getInteractiveOrbit();
    if (!orbit) return;
    this.#setUserState(applyOrbitZoom(this.#orbitState, pinchDistance(gesture.context, gesture.scale), orbit), 'touch');
  }
  ...
  #handleSpatialKey(command: SpatialKeyCommand): SpatialKeyHandling {
    ...
    const next =
      command.kind === 'direction'
        ...
        : orbit
          ? applyOrbitKey(this.#orbitState, command.key, orbit)
          : null;
  ...
  #getDirectionalKeyState(
    ...
    const orbit = this.#getInteractiveOrbit();
    return orbit ? applyOrbitKey(this.#orbitState, command.key, orbit) : null;
  }
  ...
  #panByPixels(
    ...
    const azimuth = next.target.heading + next.offset.azimuth;
  ...
  #worldUnitsPerPixel(
    ...
    return top
      ? (state.projection.mode === 'orthographic' ? state.projection.frustumHeight : top.altitude) / height
      : (2 *
          state.offset.distance *
          Math.tan(state.projection.mode === 'perspective' ? state.projection.verticalFieldOfView / 2 : 0.5)) /
          height;
  }

  #setUserState(state: OrbitCameraState, source: SceneCameraChangeSource): void {
  ...
  #syncUserStateToBehaviors(state: OrbitCameraState): void {
    for (const { behavior, contribution } of this.#behaviors) {
      if (contribution.kind === 'orbit') {
        setCameraRuntimeProperty(behavior, 'distance', state.offset.distance);
        setCameraRuntimeProperty(behavior, 'polarAngle', state.offset.polarAngle);
        setCameraRuntimeProperty(behavior, 'azimuth', state.offset.azimuth);
        setCameraRuntimeProperty(behavior, 'projection', state.projection.mode);
        if (state.projection.mode === 'perspective')
          setCameraRuntimeProperty(behavior, 'verticalFieldOfView', state.projection.verticalFieldOfView);
        else setCameraRuntimeProperty(behavior, 'frustumHeight', state.projection.frustumHeight);
        this.#syncUserTarget(behavior, contribution.fields, state.target);
      } else if (contribution.kind === 'top') {
        setCameraRuntimeProperty(
          behavior,
          'frustumHeight',
          state.projection.mode === 'orthographic' ? state.projection.frustumHeight : behavior.frustumHeight
        );
        this.#syncUserTarget(behavior, contribution.fields, state.target);
      }
    }
  ...
  #syncUserTarget(behavior: SceneCamera, fields: readonly CameraField[], target: CameraTarget): void {
    if (fields.includes('target.position')) setCameraRuntimeProperty(behavior, 'target', [...target.position]);
    if (fields.includes('target.heading')) setCameraRuntimeProperty(behavior, 'heading', target.heading);
  }
}

function frameOrientation(matrix: Float64Array) {
...
function cameraSnapshotEntryMatchesCurrent(entry: CameraBehaviorSnapshot, camera: SceneCamera): boolean {
  if (entry.behavior !== camera || entry.revision !== getCameraInputRevision(camera)) return false;
  const contribution = entry.contribution;
  if (!contribution) return camera.disabled;
  if (camera.disabled) return false;
  if (contribution.kind === 'orbit' || contribution.kind === 'top') {
    return sameTuple(contribution.target.position, camera.target);
  }
  return (
    contribution.kind !== 'pose' ||
    (sameTuple(contribution.pose.position, camera.position) &&
      sameNormalizedQuaternion(contribution.pose.orientation, camera.orientation))
  );
}

function sameTuple(left: readonly number[], right: readonly number[]): boolean {
  return left.length === right.length && left.every((value, index) => sameNumber(value, right[index] ?? Number.NaN));
}

function sameNormalizedQuaternion(left: Readonly<Quaternion>, right: Readonly<Quaternion>): boolean {
  const magnitude = Math.hypot(right[0], right[1], right[2], right[3]);
  return (
    magnitude > 0 &&
    sameNumber(left[0], right[0] / magnitude) &&
    sameNumber(left[1], right[1] / magnitude) &&
    sameNumber(left[2], right[2] / magnitude) &&
    sameNumber(left[3], right[3] / magnitude)
  );
}
...
function pointerCameraSource(event: PointerEvent): Extract<SceneCameraChangeSource, 'pointer' | 'touch'> {
