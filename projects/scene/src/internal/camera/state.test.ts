...
import { DEFAULT_CAMERA_STATE, copyCameraState } from '../math/camera.js';
...
describe('camera state resolution', () => {
  it('applies behavior contributions over prior state', () => {
      ...
      contributions: [
        {
          ...
          patch: {
            ...
            offset: { distance: 8 },
            projection: { mode: 'ortho', frustumHeight: 10 }
          }
        }
      ]
    });
    ...
    expect(resolved.projection).toEqual({ mode: 'ortho', frustumHeight: 10 });
  });

  it('returns independent state snapshots', () => {
    const prior = copyCameraState(DEFAULT_CAMERA_STATE);
  ...
  it('applies multiple contribution patches in order', () => {
    const resolved = resolveCameraState({
      contributions: [
        ...
        { fields: ['projection'], patch: { projection: { mode: 'ortho', frustumHeight: 30 } } }
      ]
    });
    expect(resolved).toEqual({
      target: { position: [1, 0, 0], heading: 1 },
      offset: { distance: 9, phi: 1, theta: 2 },
      projection: { mode: 'ortho', frustumHeight: 30 }
    });
  });
});
