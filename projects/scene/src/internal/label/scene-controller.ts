...
import { frameHasTransform, getFrameWorldMatrix, getNamedSceneFrame, isFrameChainValid } from '../frame/state.js';
...
export class LabelSceneController {
  ...
  synchronize(
    ...
    for (const label of this.#labels) {
      ...
      const frame = configuration.frame ? this.#resolveFrame(configuration.frame) : undefined;
  ...
  #validate(label: HTMLElement, child: HTMLElement | undefined): boolean {
    const configuration = getLabelConfiguration(label);
    const frame = configuration.frame === null ? undefined : this.#resolveFrame(configuration.frame);
    setLabelSceneState(label, { occluded: false, stale: frame !== undefined && !frameHasTransform(frame) });
  ...
  #resolveFrame(name: string): HTMLElement | undefined {
