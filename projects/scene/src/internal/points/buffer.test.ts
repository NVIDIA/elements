...
describe('point buffer', () => {
  it('should allocate fixed capacity and support allocation-free writes', () => {
    ...
    expect(points.mutableBytes).toHaveLength(2 * POINT.stride);
    expect(points.capacity).toBe(2);
    expect(points.count).toBe(0);
    expect(readPoint(points.mutableBytes, 1)).toEqual({ color: [1, 1, 1, 1], position: [0, 0, 0] });

    points.set(0, { color: 'cyan', position: [1, 2, 3] }).set(1, { color: [1, 0, 0, 0.5] });

    expect(points.count).toBe(2);
    expect(readPoint(points.mutableBytes, 0)).toEqual({ color: [0, 1, 1, 1], position: [1, 2, 3] });
    expect(readPoint(points.mutableBytes, 1).color).toEqual([1, 0, 0, 128 / 255]);
  ...
  it('should return stable mutable handles on demand', () => {
    ...
    expect(readPoint(points.mutableBytes, 0).position).toEqual([4, 5, 6]);
  ...
  it('should change the active prefix without resizing storage', () => {
    const points = new PointBuffer({ capacity: 3 });
    points.add({ position: [1, 0, 0] });
    points.setCount(0);
    expect(points.count).toBe(0);
    expect(points.mutableBytes).toHaveLength(3 * POINT.stride);
    points.setCount(3);
    expect(points.count).toBe(3);
    expect(() => points.setCount(4)).toThrow(RangeError);
  });
});
