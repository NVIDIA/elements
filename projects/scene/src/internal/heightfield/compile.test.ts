...
import { compileHeightfield, prepareHeightfield, recomputeHeightfield, validateHeightfieldGrid } from './compile.js';
import { PREPARATION_CHUNK_SIZE, type PreparationContext } from '../preparation.js';
import { prepareHeightfieldIndices } from './topology.js';
...
describe('heightfield compilation', () => {
  ...
  it('rejects invalid grid shapes, values, and unsafe allocation dimensions', () => {
    ...
    expect(() => validateHeightfieldGrid(null as unknown as HeightfieldGrid)).toThrow(RangeError);
    expect(() => validateHeightfieldGrid('grid' as unknown as HeightfieldGrid)).toThrow(RangeError);
    expect(() => validateHeightfieldGrid({ ...grid, spacing: Number.NaN })).toThrow(RangeError);
    ...
    expect(() => validateHeightfieldGrid({ ...grid, heights: [0, 0, 0, 0] as unknown as Float32Array })).toThrow(
      RangeError
    );
    ...
    expect(() => validateHeightfieldGrid({ ...grid, origin: 'origin' as unknown as [number, number] })).toThrow(
      RangeError
    );
    expect(() => validateHeightfieldGrid({ ...grid, origin: [0] as unknown as [number, number] })).toThrow(RangeError);
    expect(() => validateHeightfieldGrid({ ...grid, origin: [Number.POSITIVE_INFINITY, 0] })).toThrow(RangeError);
    expect(() => validateHeightfieldGrid({ ...grid, origin: [0, Number.POSITIVE_INFINITY] })).toThrow(RangeError);
    expect(() => validateHeightfieldGrid({ ...grid, colors: [0, 0, 0, 0] as unknown as Uint8Array })).toThrow(
      RangeError
    );
  ...
  it('prepares a validated grid after yielding and matches synchronous compilation', async () => {
    const grid = withColors(
      gridFrom(3, 3, (row, column) => row + column),
      [0, 64, 128, 255]
    );
    const deferred = createDeferredContext();
    let settled = false;
    const pending = prepareHeightfield(grid, deferred.context).then(result => {
      settled = true;
      return result;
    });
    ...
    expect(await pending).toEqual(compileHeightfield(grid));
  });

  it('discards bounded preparation at every compilation stage', async () => {
    const grid = gridFrom(2, PREPARATION_CHUNK_SIZE / 2 + 1, () => 0);
    ...
    for (const checks of cancellationChecks) {
      await expect(prepareHeightfield(grid, createSequencedContext(...checks))).resolves.toBeUndefined();
    }
  });
  ...
  it('discards bounded color preparation while normalizing samples', async () => {
    ...
    const checksBeforeColors = [true, true, true, true, true];

    await expect(
      prepareHeightfield(grid, createSequencedContext(...checksBeforeColors, false))
    ).resolves.toBeUndefined();
    await expect(
      prepareHeightfield(grid, createSequencedContext(...checksBeforeColors, true, true, true, true, false))
    ).resolves.toBeUndefined();
  });

  it('discards bounded topology preparation between index chunks', async () => {
    await expect(
      prepareHeightfieldIndices(2, PREPARATION_CHUNK_SIZE + 1, createSequencedContext(true, false))
    ).resolves.toBeUndefined();
  });
});
