...
describe('heightfield queries', () => {
  ...
  it('samples the same top-right-to-bottom-left triangles as rendered terrain', () => {
    const saddle: HeightfieldGrid = {
      spacing: 1,
      columns: 2,
      rows: 2,
    ...
    expect(heightAt(saddle, 0.5, 0.5)).toBe(1);
    expect(surfaceHeightAt(saddle, 0.5, 0.5)).toBe(0);
    expect(surfaceHeightAt(saddle, 0.25, 0.25)).toBe(0);
    expect(surfaceHeightAt(saddle, 0.75, 0.75)).toBe(2);
    expect(surfaceHeightAt(saddle, 1, 1)).toBe(4);
    expect(surfaceHeightAt(saddle, 1.000_001, 1)).toBeUndefined();
    expect(surfaceNormalAt(saddle, 0.5, 0.5)).toEqual([0, 0, 1]);
    expect(surfaceSlopeAt(saddle, 0.5, 0.5)).toBe(0);
    expect(drapeToSurface(saddle, new Float32Array([0.5, 0.5, 9]), 0.25)).toEqual(new Float32Array([0.5, 0.5, 0.25]));
    expect(surfaceHeightAt(bilinearGrid, 11, -4)).toBe(3);
    expect(surfaceHeightAt(bilinearGrid, 11.5, -3.5)).toBe(6.5);
    expect(surfaceHeightAt(bilinearGrid, 12, -3)).toBe(10);
    expectCloseArray(surfaceNormalAt(bilinearGrid, 10.5, -4.5)!, normalize(-1, -2, 1), 12);
    expect(drapeToSurface(bilinearGrid, new Float32Array([11, -4, 99]))).toEqual(new Float32Array([11, -4, 3]));
  });
});
