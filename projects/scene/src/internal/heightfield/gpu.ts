...
import {
  ...
  type SceneGPUComputePass,
  type SceneGPUComputePipeline,
  type SceneGPUDevice
} from '../gpu/platform.js';
import type { HeightfieldMeshData } from '../mesh/render-data.js';
...
export class HeightfieldGPUCompiler {
  ...
  create(
    source: HeightfieldMeshData,
    indices: Uint32Array = createHeightfieldIndices(source.rows, source.columns)
  ): HeightfieldGPUState {
    ...
    try {
      ({ colors, geometry, heights, parameters } = this.#createResources(source, vertexCount, indices));
      const state = {
  ...
  #createResources(
    ...
    let parameters: SceneGPUBuffer | undefined;
    try {
      geometry = createGeneratedMeshGeometryResources(this.#device, vertexCount, indices);
...
export function supportsHeightfieldGPU(device: SceneGPUDevice): device is SceneGPUComputeDevice {
