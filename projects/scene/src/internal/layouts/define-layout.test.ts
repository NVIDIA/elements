...
import { FIELD_BYTE_WIDTHS, defineLayout, getFieldOffset } from './define-layout.js';
...
describe(defineLayout.name, () => {
  ...
  it('should resolve field offsets and reject missing fields', () => {
    const layout = defineLayout('test.field-offset', { value: { type: 'f32', offset: 4 } }, { stride: 8 });

    expect(getFieldOffset(layout, 'value')).toBe(4);
    expect(() => getFieldOffset(layout, 'missing')).toThrow(
      new TypeError('Layout "test.field-offset" has no field named "missing".')
    );
  });
});
...
describe('built-in layouts', () => {
  ...
  it.each([
    [POINT, 'nve.point'],
    [TRIANGLE_VERTEX, 'nve.triangle-vertex']
