...
describe('vertex stream buffer', () => {
  ...
  it('revalidates default line records on replacement and commit without changing capacity', () => {
    const source = new Uint8Array(LINE_VERTEX.stride * 2);
    writeLineVertex(source, 0, { position: [1, 2, 3], color: [1, 1, 1, 1] });
    writeLineVertex(source, 1, { position: [4, 5, 6], color: [1, 1, 1, 0.5] });
    const stream = new VertexStreamBuffer(LINE_VERTEX);

    stream.replace(source);
    expect(stream.toRenderData({ consumeUploadRanges: false })).toMatchObject({ opaque: true, transparent: true });

    writeLineVertex(source, 0, { position: [7, 8, 9], color: [1, 1, 1, 0.5] });
    stream.replace(source);
    ...
    writeLineVertex(source, 0, { position: [10, 11, 12], color: [1, 1, 1, 1] });
    stream.commit(0, 1);
    expect(stream.toRenderData({ consumeUploadRanges: false })).toMatchObject({ opaque: true, transparent: true });
  });

  it('rebuilds default line summaries after widespread classification changes', () => {
    const count = 1_100;
    const opaque = new Uint8Array(LINE_VERTEX.stride * count);
    const transparent = new Uint8Array(LINE_VERTEX.stride * count);
    for (let index = 0; index < count; index += 1) {
      writeLineVertex(opaque, index, { position: [index, 0, 0], color: [1, 1, 1, 1] });
      writeLineVertex(transparent, index, { position: [index, 0, 0], color: [1, 1, 1, 0.5] });
    }
    const stream = new VertexStreamBuffer(LINE_VERTEX);
  ...
  it('validates borrowed prefixes and falls back when prepared sources are incompatible', () => {
    const source = new Uint8Array(LINE_VERTEX.stride * 2);
    writeLineVertex(source, 0, { position: [1, 2, 3] });
    writeLineVertex(source, 1, { position: [4, 5, 6] });
    const stream = new VertexStreamBuffer(LINE_VERTEX, { requireCountMultipleOf: 2 });
    expect(stream.sourceRecordsAreValid(source, 2)).toBe(true);
    expect(stream.sourceRecordsAreValid(source, 1)).toBe(false);
    expect(stream.sourceRecordsAreValid(source, 3)).toBe(false);
    expect(stream.sourceRecordsAreValid(new Uint8Array(1), 0)).toBe(false);
    expect(stream.sourceRecordsAreValid(source, -1)).toBe(false);

    stream.replace(source);
    const prepared = stream.createPreparedSnapshot();
    if (!prepared) throw new Error('Expected a prepared snapshot.');
    const target = new VertexStreamBuffer(POINT);
    target.replacePrepared(new Uint8Array(POINT.stride), undefined, prepared);
    expect(target.capacity).toBe(1);
  });

  it('revalidates custom record rules during replacement and ranged commits', () => {
    const source = new Uint8Array(POINT.stride * 2);
    writePoint(source, 0, { position: [1, 2, 3] });
    writePoint(source, 1, { position: [4, 5, 6] });
    const stream = new VertexStreamBuffer(POINT, {
      opaqueRecord: (_records, byteOffset) => byteOffset === 0,
      transparentRecord: (_records, byteOffset) => byteOffset !== 0,
      validateRecord: (records, byteOffset) => Number.isFinite(records.getFloat32(byteOffset, true))
    });

    stream.replace(source);
    expect(stream.toRenderData({ consumeUploadRanges: false })).toMatchObject({ opaque: true, transparent: true });

    const invalidSource = source.slice();
    new DataView(invalidSource.buffer).setFloat32(0, Number.NaN, true);
    expect(stream.sourceRecordsAreValid(invalidSource, 2)).toBe(false);
    ...
    stream.commit(0, 1);
    expect(stream.toRenderData({ consumeUploadRanges: false })).toMatchObject({ ready: true, opaque: true });
  });

  it('revalidates no-color layouts and reports empty, count, and range edge cases', () => {
    const noColor = defineLayout(
      'nve.test-no-color-revalidation',
      { position: { type: 'f32x3', offset: 0 } },
      { stride: 12 }
    );
    const noColorSource = new Uint8Array(noColor.stride);
    const noColorStream = new VertexStreamBuffer(noColor);
    noColorStream.replace(noColorSource);
    noColorStream.replace(noColorSource);
    noColorStream.commit(0, 1);
    expect(noColorStream.toRenderData({ consumeUploadRanges: false })).toMatchObject({
      opaque: true,
      transparent: false
    });
    expect(noColorStream.getUploadBytes()).not.toBeNull();

    const customNoColorStream = new VertexStreamBuffer(noColor, { validateRecord: () => true });
    customNoColorStream.replace(noColorSource);
    customNoColorStream.replace(noColorSource);
    expect(customNoColorStream.toRenderData({ consumeUploadRanges: false })).toMatchObject({
      opaque: true,
      transparent: false
    });

    const empty = new VertexStreamBuffer(POINT);
    empty.replace(new Uint8Array());
    expect(empty.toRenderData({ consumeUploadRanges: false })).toMatchObject({ ready: true, capacity: 0 });

    const triangles = new VertexStreamBuffer(TRIANGLE_VERTEX, { requireCountMultipleOf: 3 });
    triangles.replace(new Uint8Array(TRIANGLE_VERTEX.stride * 4));
    triangles.count = 1;
    expect(triangles.hasCountIssue()).toBe(true);
    expect(() => triangles.commit(-1)).toThrow(RangeError);
    expect(() => triangles.commit(0, 5)).toThrow(RangeError);

    const uninitialized = new VertexStreamBuffer(POINT);
    expect(uninitialized.getUploadBytes()).toBeNull();
  });
  ...
  it('validates stride and recovers a touched range through commit', () => {
    ...
    expect(stream.getUploadBytes()).toBeNull();
  ...
  it('reports triangle divisibility without throwing from count assignment', () => {
    const stream = new VertexStreamBuffer(TRIANGLE_VERTEX, { requireCountMultipleOf: 3 });
  ...
  it('handles byte-offset views, alpha prefixes, and specialized layouts', () => {
    ...
    expect(new VertexStreamBuffer(TRIANGLE_VERTEX).layout).toBe(TRIANGLE_VERTEX);
