...
describe(compilePolygon.name, () => {
  ...
  it.each([
    ...
    ['missing outer ring', {}],
    ['non-array outer ring', { outer: null }],
    [
      'non-array holes',
      {
        outer: [
    ...
    ['non-array point', { outer: [[0, 0], [1, 0], null] }],
    ['point with the wrong length', { outer: [[0, 0], [1, 0], [0]] }],
    [
      'non-number x coordinate',
      {
        outer: [
          [0, 0],
          [1, 0],
  ...
  it('should remove consecutive duplicate vertices', () => {
    const compiled = compilePolygon({
      outer: [
        [0, 0],
        [4, 0],
        [4, 0],
    ...
    expect(compiled.outer).toHaveLength(4);
    expect(triangleArea(compiled.positions, compiled.indices)).toBe(16);
  });
  ...
  return triangle.some((start, index) => {
    ...
    return rings.some(ring =>
      ring.some((boundaryStart, boundaryIndex) => {
        ...
        return properIntersection({ boundaryEnd, boundaryStart, end, start });
      })
    );
  });
}

function properIntersection(options: {
  readonly boundaryEnd: Point;
  readonly boundaryStart: Point;
  readonly end: Point;
  readonly start: Point;
}): boolean {
  const { boundaryEnd: fourth, boundaryStart: third, end: second, start: first } = options;
