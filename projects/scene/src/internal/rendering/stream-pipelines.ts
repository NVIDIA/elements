...
export function createStreamPipelines(device: SceneGPURenderPipelineDevice, format: string): StreamPipelines {
  return {
...
const LINE_SCENE_FIELDS = `${SCENE_FIELDS}, sourceCount: f32, segmentStart: f32, segmentCount: f32, joinStart: f32, joinCount: f32, segmentBase: f32`;

const COLOR_WGSL = /* wgsl */ `
${SRGB_TO_LINEAR_WGSL}
fn color(value: vec4f) -> vec4f { return vec4f(srgbToLinear(value.rgb) * value.a, value.a); }
`;

/** Composes stream color and ID/depth variants from their shared vertex calculations. */
export function createPointShader(options: { readonly pass: 'color' | 'pick' }): string {
...
@vertex fn vertexMain(@builtin(vertex_index) index: u32) -> Output { let point = nve_load_stream_vertex(index / 6u); let clip = scene.viewProjection * scene.frame * vec4f(point.position, 1.0); var output: Output; output.position = clip + vec4f(pointOffset(corner(index % 6u), clip), 0.0, 0.0); output.color = color(point.color); ${pick ? 'output.id = scene.pickId + index / 6u;' : ''} return output; }
${streamFragment(pick)}
`;
}

export function createTriangleShader(options: { readonly pass: 'color' | 'pick' }): string {
  const pick = options.pass === 'pick';
  return /* wgsl */ `
${streamUniforms(pick)}
${STREAM_WGSL}
@vertex fn vertexMain(@builtin(vertex_index) index: u32) -> Output { let vertex = nve_load_stream_vertex(index); var output: Output; output.position = scene.viewProjection * scene.frame * vec4f(vertex.position, 1.0); output.color = color(vertex.color); ${pick ? 'output.id = scene.pickId + index / 3u;' : ''} return output; }
${streamFragment(pick)}
`;
}

const POINT_SHADER = createPointShader({ pass: 'color' });
const TRIANGLE_SHADER = createTriangleShader({ pass: 'color' });

function streamUniforms(pick: boolean): string {
  return /* wgsl */ `
...
fn segmentIdentity(segment: u32) -> u32 { return u32(scene.segmentBase) + segment - u32(scene.segmentStart); }
fn vertexIdentity(vertex: u32) -> u32 { return (u32(scene.segmentBase) + u32(scene.sourceCount) + vertex - u32(scene.segmentStart)) % u32(scene.sourceCount); }
...
fn screenSegment(segment: u32, shape: vec2f, count: u32) -> LineOutput { let startIndex = segmentStart(segment); let identity = segmentIdentity(segment); let start = nve_load_line_vertex(startIndex); let end = nve_load_line_vertex(segmentEnd(startIndex, count)); if (start.width <= 0.0) { return empty(identity); } let startClip = projected(start.position); let endClip = projected(end.position); let unit = direction2(pixels(startClip), pixels(endClip)); if (dot(unit, unit) < 0.000001) { return empty(identity); } let normal = vec2f(-unit.y, unit.x); let halfWidth = start.width * scene.pixelRatio * 0.5; let base = select(startClip, endClip, shape.x > 0.5); let length = distance(pixels(startClip), pixels(endClip)); return outputAt(clipOffset(base, normal * halfWidth * shape.y), start.color, 0.0, shape.x * length, length, vec2f(start.dash, start.gap) * scene.pixelRatio, identity); }
fn worldSegment(segment: u32, shape: vec2f, count: u32) -> LineOutput { let startIndex = segmentStart(segment); let identity = segmentIdentity(segment); let start = nve_load_line_vertex(startIndex); let end = nve_load_line_vertex(segmentEnd(startIndex, count)); if (start.width <= 0.0) { return empty(identity); } let delta = end.position - start.position; let sideValue = cross(delta, start.normal); if (dot(delta, delta) < 0.000001 || dot(sideValue, sideValue) < 0.000001) { return empty(identity); } let side = normalize(sideValue); let base = select(start.position, end.position, shape.x > 0.5); let length = distance(start.position, end.position); return outputAt(projected(base + side * start.width * 0.5 * shape.y), start.color, shape.x * length, 0.0, length, vec2f(start.dash, start.gap), identity); }
fn joinPoint(join: u32) -> u32 { return select(join + 1u, join, isLoop()); }
fn screenJoin(join: u32, shape: u32, count: u32) -> LineOutput { let pointIndex = joinPoint(join); let identity = vertexIdentity(pointIndex); let previousIndex = select(pointIndex - 1u, count - 1u, pointIndex == 0u); let nextIndex = select(pointIndex + 1u, 0u, pointIndex + 1u == count); let previous = nve_load_line_vertex(previousIndex); let point = nve_load_line_vertex(pointIndex); if (previous.width <= 0.0 || point.width <= 0.0 || previous.gap > 0.0 || point.gap > 0.0) { return empty(identity); } let pointClip = projected(point.position); let incoming = direction2(pixels(projected(previous.position)), pixels(pointClip)); let outgoing = direction2(pixels(pointClip), pixels(projected(nve_load_line_vertex(nextIndex).position))); let turn = incoming.x * outgoing.y - incoming.y * outgoing.x; if (dot(incoming,incoming) < 0.000001 || dot(outgoing,outgoing) < 0.000001 || abs(turn) < 0.000001) { return empty(identity); } let first = vec2f(-incoming.y,incoming.x); let second = vec2f(-outgoing.y,outgoing.x); let side = select(1.0,-1.0,turn>0.0); let offset = joinedOffset2(first, second, previous.width * scene.pixelRatio * 0.5, point.width * scene.pixelRatio * 0.5, side, shape); return outputAt(clipOffset(pointClip, offset), point.color, 0.0, 0.0, 0.0, vec2f(0.0), identity); }
fn worldJoin(join: u32, shape: u32, count: u32) -> LineOutput { let pointIndex = joinPoint(join); let identity = vertexIdentity(pointIndex); let previousIndex = select(pointIndex - 1u, count - 1u, pointIndex == 0u); let nextIndex = select(pointIndex + 1u, 0u, pointIndex + 1u == count); let previous = nve_load_line_vertex(previousIndex); let point = nve_load_line_vertex(pointIndex); let next = nve_load_line_vertex(nextIndex); if (previous.width <= 0.0 || point.width <= 0.0 || previous.gap > 0.0 || point.gap > 0.0) { return empty(identity); } let incoming = direction3(previous.position, point.position); let outgoing = direction3(point.position, next.position); let firstValue = cross(incoming, previous.normal); let secondValue = cross(outgoing, point.normal); if (dot(firstValue,firstValue) < 0.000001 || dot(secondValue,secondValue) < 0.000001) { return empty(identity); } let first = normalize(firstValue); let second = normalize(secondValue); var planeNormal = point.normal; let normalSum = previous.normal + point.normal; if (dot(normalSum,normalSum) > 0.000001) { planeNormal = normalize(normalSum); } let turn = dot(cross(incoming,outgoing), planeNormal); if (abs(turn) < 0.000001) { return empty(identity); } let side = select(1.0,-1.0,turn>0.0); let offset = joinedOffset3(first, second, previous.width * 0.5, point.width * 0.5, side, shape); return outputAt(projected(point.position + offset), point.color, 0.0, 0.0, 0.0, vec2f(0.0), identity); }
...
@vertex fn vertexMain(@builtin(vertex_index) index: u32) -> LineOutput { let count = u32(scene.count); let segments = u32(scene.segmentCount); let segmentVertices = segments * 6u; if (index < segmentVertices) { let segment = u32(scene.segmentStart) + index / 6u; if (scene.worldUnit > 0.5) { return worldSegment(segment, corner(index % 6u), count); } return screenSegment(segment, corner(index % 6u), count); } let join = (index - segmentVertices) / 3u; if (join >= u32(scene.joinCount)) { return empty(0u); } let point = u32(scene.joinStart) + join; if (scene.worldUnit > 0.5) { return worldJoin(point, (index-segmentVertices)%3u, count); } return screenJoin(point, (index-segmentVertices)%3u, count); }
...
`;
  return `struct Scene { ${LINE_SCENE_FIELDS}${pickField} }\n@group(0) @binding(0) var<uniform> scene: Scene;\n${LINE_BODY}\n${fragment}`;
}

const LINE_SHADER = createLineShader(false);
