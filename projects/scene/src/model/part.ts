...
export class ScenePart extends LitElement {
  ...
  get shape(): ScenePrimitiveKind {
    return this.#shape;
  }

  set shape(value: ScenePrimitiveKind) {
    this.#setPartProperty({ assign: next => (this.#shape = next), name: 'shape', previous: this.#shape, value });
  }

  #shape: ScenePrimitiveKind = 'cube';
  ...
  #color: SceneColor = '#ffffff';

  render() {
    return nothing;
  }

  #setPartProperty<T extends SceneColor | ScenePrimitiveKind | Vec3 | Quaternion>(options: {
...
function partPropertyValuesEqual(
  left: SceneColor | ScenePrimitiveKind | Vec3 | Quaternion,
  right: SceneColor | ScenePrimitiveKind | Vec3 | Quaternion
): boolean {
