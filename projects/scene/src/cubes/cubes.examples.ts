...
export const BufferSource = {
  render: () => html`
    <nve-scene aria-label="buffered cubes scene">
      <nve-scene-camera behavior="orbit"></nve-scene-camera>
      <nve-scene-gridlines></nve-scene-gridlines>
    ...
    <script type="module">
      ...
      const cubes = new CubeBuffer({ capacity: 3 });
      cubes.add({ position: [-2.5, 0, 0.375], size: [0.75, 0.75, 0.75], color: 'cyan' });
      cubes.add({ position: [0, 0, 0.5], size: [1, 1, 1], color: 'magenta' });
      cubes.add({ position: [2.5, 0, 0.75], size: [1.5, 1.5, 1.5], color: 'yellow' });
      document.querySelector('#buffered-cubes').source = cubes;
    </script>
  `
};
...
export const Volumes = {
  render: () => html`
    ...
    <script type="module">
      ...
      document.querySelector('#streamed-volumes').source = cubes;
    </script>
  `
};

/**
 * @summary A fixed-capacity sensor grid maps values through the Virdis scale while reusing one cube buffer and committing only each incoming row. Use this pattern for frequent scalar telemetry updates when scene topology stays stable.
 */
export const Streaming = {
  render: () => html`
    ...
    <script type="module">
      ...
      const columns = 12;
      const rows = 8;
      const cubes = new CubeBuffer({ capacity: columns * rows });
      const cubeHandles = Array.from({ length: cubes.capacity }, () => cubes.add());
      ...
      const writeRow = (row, time) => {
        for (let column = 0; column < columns; column += 1) {
          ...
          const cube = cubeHandles[row * columns + column];
      ...
      const streamRow = time => {
        ...
        cubeLayer.publish({ start: nextRow * columns, count: columns });
  ...
  render: () => html`
    <section nve-layout="column gap:sm" style="position: relative;">
      <nve-scene aria-label="performance scene">
        <nve-scene-camera behavior="orbit" target="[0,0,0.4]" distance="6.7" polar-angle="1.33" azimuth="-0.5"></nve-scene-camera>
    ...
    <script type="module">
      ...
      const cubeLayer = document.querySelector('#performance-cubes');
      ...
      const animate = time => {
        ...
        cubeLayer.publish({ start: 0, count: instanceCount });
