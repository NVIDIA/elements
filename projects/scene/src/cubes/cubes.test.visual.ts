      ...
      /* html */ `
        ...
        <script type="module">
          ...
          for (let index = 0; index < count; index += 1) {
            ...
            markers.mutableBytes.set([118, 185, 0, 255], offset + 40);
          }
          markers.setCount(count);
          document.querySelector('#compacted').source = markers;
    ...
    const outlinePixels = await visualRunner.inspect(
      ...
      /* html */ `
        ...
        <script type="module">
          ...
          document.querySelector('#streamed-outline').source = markers;
        </script>
      `,
      page =>
        page.evaluate(async () => {
          ...
          for (let index = 0; index < data.length; index += 4) {
            if (isCyanPixel(data, index)) count += 1;
          }
          return count;

          function isCyanPixel(values: ArrayLike<number>, index: number): boolean {
            const red = values[index] ?? 0;
            return (values[index + 1] ?? 0) > red + 20 && (values[index + 2] ?? 0) > red + 20;
          }
        })
    );
