...
describe('scene lines visual runtime', () => {
  test('renders pixel widths and excludes dash gaps from picking', async () => {
    const pixels = await webgpuVisualRunner.inspect(
      ...
      `<nve-scene aria-label="lines" style="width:96px;height:48px;background:rgb(0 0 0)"><nve-scene-camera behavior="top" target="[0,0,0]" height="2"></nve-scene-camera><nve-scene-lines id="pixel-lines" topology="segments" width-unit="pixel"></nve-scene-lines></nve-scene><script type="module">
        window.__lineErrors = [];
        ...
        const pixelBytes = new Uint8Array(LINE_VERTEX.stride * 4);
        writeLineVertex(pixelBytes, 0, { position: [-0.8, -0.45, 0], color: [1, 0, 0, 1], width: 2 });
        writeLineVertex(pixelBytes, 1, { position: [0.8, -0.45, 0] });
        writeLineVertex(pixelBytes, 2, { position: [-0.8, 0, 0], color: [1, 0, 0, 1], width: 7, dash: 8, gap: 6 });
        writeLineVertex(pixelBytes, 3, { position: [0.8, 0, 0] });
        document.querySelector('#pixel-lines').vertices = pixelBytes;
  ...
  test('renders a world-width line from its frame-local normal', async () => {
    const pixels = await webgpuVisualRunner.inspect(
      ...
      `<nve-scene aria-label="world line" style="width:64px;height:32px;background:rgb(0 0 0)"><nve-scene-camera behavior="top" target="[0,0,0]" height="2"></nve-scene-camera><nve-scene-lines id="world-line"></nve-scene-lines></nve-scene><script type="module">
        window.__lineErrors = [];
        const originalError = console.error;
        console.error = (...values) => { window.__lineErrors.push(values.map(String).join(' ')); originalError(...values); };
        import { LINE_VERTEX, writeLineVertex } from '@nvidia-elements/scene';
        import '@nvidia-elements/scene/camera/define.js';
        import '@nvidia-elements/scene/lines/define.js';
        const bytes = new Uint8Array(LINE_VERTEX.stride * 2);
        writeLineVertex(bytes, 0, { position: [-0.8, 0, 0], color: [1, 0, 0, 1], normal: [0, 0, 1], width: 0.12 });
        writeLineVertex(bytes, 1, { position: [0.8, 0, 0] });
        document.querySelector('#world-line').vertices = bytes;
      </script>`,
      page =>
        page.evaluate(async () => {
          const scene = document.querySelector('nve-scene');
          ...
          return { colored, errors: window.__lineErrors };
        })
    );
    expect(pixels.errors).toEqual([]);
