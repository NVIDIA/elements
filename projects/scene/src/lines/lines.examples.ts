...
export const Default = {
  render: () => html`
    <nve-scene aria-label="Line topology examples" style="min-height: 480px">
      <nve-scene-camera behavior="orbit" target="[0,0,0]" height="10"></nve-scene-camera>
    ...
    <script type="module">
      ...
      const setVertices = (id, records) => {
        ...
        records.forEach((record, index) => vertices.set(index, record));
      ...
      setVertices('topology-strip', [
        { position: [-3.5, 2.5, 0.05], color: cyan, width: 5 },
        { position: [-2, 1.2, 0.05], color: cyan, width: 5 },
        { position: [-0.5, 2.5, 0.05], color: cyan, width: 5 }
      ]);
      setVertices('topology-loop', [
        { position: [1, 1.2, 0.05], color: magenta, width: 5 },
        { position: [3.5, 1.2, 0.05], color: magenta, width: 5 },
        { position: [2.25, 3.2, 0.05], color: magenta, width: 5 }
      ]);
      setVertices(
        'topology-circle',
      ...
      setVertices('topology-segments', [
        { position: [0.5, -2.7, 0.05], color: cyan, width: 5 },
        { position: [3.5, -2.7, 0.05] },
        { position: [0.5, -1.8, 0.05], color: magenta, width: 5 },
...
export const SegmentStyles = {
  render: () => html`
    ...
    <script type="module">
      ...
      const yellow = [255, 255, 0, 1];
      const records = [
        { position: [-4, 1, 0.05], color: cyan, width: 3 },
        { position: [-2, 2, 0.05], color: cyan, width: 8, dash: 18, gap: 10 },
        { position: [0, 1, 0.05], width: 0 },
        { position: [1, -1, 0.05], color: magenta, width: 5 },
        { position: [4, -1, 0.05] }
      ];
      const vertices = new LineVertexBuffer({ capacity: records.length });
      records.forEach((record, index) => vertices.set(index, record));
  ...
  render: () => html`
    ...
    <script type="module">
      ...
      const setSegment = (id, start, end, style) => {
        ...
        vertices.set(0, { position: start, ...style });
        vertices.set(1, { position: end });
        document.querySelector('#' + id).vertices = vertices;
      };
...
export const StreamingTrail = {
  render: () => html`
    <nve-scene id="streaming-trail-scene" aria-label="Autonomous mobile robot route history" style="min-height: 480px">
      <nve-scene-camera behavior="orbit" target="[0,0,0]" height="14"></nve-scene-camera>
    ...
    <script type="module">
      ...
      function appendSample(position) {
        if (sampleCount === maxSamples) {
          vertices.bytes.copyWithin(0, LINE_VERTEX.stride);
          sampleCount -= 1;
        }
        ...
        for (let index = 0; index < sampleCount; index += 1) {
          ...
          vertices.bytes[index * LINE_VERTEX.stride + colorOffset + 3] = Math.round(alpha * 255);
        }
