...
describe(ScenePolygon.metadata.tag, () => {
  ...
  it('should expose the public geometry, color, layout, and registration contracts', () => {
    ...
    expect(polygon.geometry).toBeNull();
  ...
  it('should propagate base and marker alpha while supporting buffers, counts, and publication', () => {
    const polygon = new ScenePolygon();
    ...
    first.position.x = 2;
    polygon.publish({ count: 1, start: 0 });
  ...
  it('should clear geometry with null, reject undefined, and normalize invalid colors', () => {
    const polygon = new ScenePolygon();
    polygon.geometry = square;
    polygon.geometry = null;
    expect(takePolygonLayerRenderData(polygon).ready).toBe(false);
    polygon.geometry = square;
    expect(() => Reflect.set(polygon, 'geometry', undefined)).toThrow(TypeError);
    expect(takePolygonLayerRenderData(polygon).ready).toBe(true);
