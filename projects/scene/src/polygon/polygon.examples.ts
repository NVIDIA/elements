...
/**
 * @summary Three polygon instances share one geometry while a packed buffer supplies their transforms and colors. Use a MarkerBuffer for large or frequently updated instance sets.
 */
export const BufferSource = {
  render: () => html`
    ...
    <script type="module">
      ...
      const polygon = document.querySelector('#buffered-polygons');
      polygon.color = 'white';
      polygon.geometry = {
        outer: [[-0.25, -1.2], [0.25, -1.2], [0.25, 0], [0.8, 0], [0, 1.2], [-0.8, 0], [-0.25, 0]]
      };

      const markers = new MarkerBuffer({ capacity: 3 });
      markers.add({ position: [-2.2, 0, 0.01], scale: [0.5, 0.5, 1], color: 'cyan' });
      markers.add({ position: [0, 0, 0.01], scale: [0.5, 0.5, 1], color: 'magenta' });
      markers.add({ position: [2.2, 0, 0.01], scale: [0.5, 0.5, 1], color: 'yellow' });
      polygon.source = markers;
    </script>
  `
};
