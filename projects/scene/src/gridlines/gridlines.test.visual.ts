...
describe('scene gridlines visual runtime', () => {
  test('keeps opaque coplanar and front triangles ahead of the positively biased grid', async () => {
    const samples = await webgpuVisualRunner.inspect('scene-gridlines-depth-bias-t2', depthTemplate(), page =>
      page.evaluate(async () => {
        ...
        async function readScene(scene: Element): Promise<{ inside: ColorCounts; outside: ColorCounts }> {
          ...
          return {
            ...
            // (-1, 0.25, 0) and (1, 0.25, 0), not values from production camera helpers.
            inside: countColors(context, 81, 83),
            outside: countColors(context, 125, 107)
          };
        }
        ...
        function countColors(context: CanvasRenderingContext2D, centerX: number, centerY: number): ColorCounts {
          ...
          for (let y = centerY - 1; y <= centerY + 1; y += 1) {
            for (let x = centerX - 1; x <= centerX + 1; x += 1) {
              const [red, greenChannel, blue] = context.getImageData(x, y, 1, 1).data;
    ...
    // The selected single vertical grid lines map to about (81, 83) and (125, 107) in
    // 200 CSS px. The opaque magenta quad covers only the first point; the second proves
    // the grid remains visible outside its footprint in every depth case.
...
function depthTemplate(): string {
  return /* html */ `
    <nve-scene aria-label="Coplanar grid" style="width: 200px; height: 200px; background: rgb(0 0 0)">
    ...
    <nve-scene aria-label="Front grid" style="width: 200px; height: 200px; background: rgb(0 0 0)">
      <nve-scene-camera behavior="orbit" target="[0,0,0]" heading="0" distance="12" phi="${Math.PI / 12}" theta="${-Math.PI / 3}" projection="ortho" frustum-height="8"></nve-scene-camera>
      <nve-scene-gridlines spacing="0.5" count="4" color="#00ff00"></nve-scene-gridlines>
      <nve-scene-triangles data-height="0.01"></nve-scene-triangles>
    </nve-scene>
    <nve-scene aria-label="Behind grid" style="width: 200px; height: 200px; background: rgb(0 0 0)">
      <nve-scene-camera behavior="orbit" target="[0,0,0]" heading="0" distance="12" phi="${Math.PI / 12}" theta="${-Math.PI / 3}" projection="ortho" frustum-height="8"></nve-scene-camera>
      <nve-scene-gridlines spacing="0.5" count="4" color="#00ff00"></nve-scene-gridlines>
      <nve-scene-triangles data-height="-0.01"></nve-scene-triangles>
    </nve-scene>
    <nve-scene aria-label="Far-front grid control" style="width: 200px; height: 200px; background: rgb(0 0 0)">
      <nve-scene-camera behavior="orbit" target="[0,0,0]" heading="0" distance="12" phi="${Math.PI / 12}" theta="${-Math.PI / 3}" projection="ortho" frustum-height="8"></nve-scene-camera>
      <nve-scene-gridlines spacing="0.5" count="4" color="#00ff00"></nve-scene-gridlines>
