...
describe('scene frame visual runtime', () => {
  test('should resolve the current frame transform in the real WebGPU scene profile', async () => {
    const result = await visualRunner.inspect(
      ...
      /* html */ `
        <nve-scene aria-label="Scene" style="width: 512px; height: 512px; background: rgb(0 0 0)">
          <nve-scene-camera behavior="orbit" target="[0,0,0]" distance="8" polar-angle="0.9" azimuth="0.7" projection="orthographic" frustum-height="8"></nve-scene-camera>
        ...
        <script type="module">
          ...
          frame.setPose({ position: [5, 0, 0], orientation: [0, 0, 0, 1] });
        </script>
      `,
      page =>
        page.evaluate(async () => {
    ...
    expect(result).toEqual({
      position: [5, 0, 0],
      pose: { position: [5, 0, 0], orientation: [0, 0, 0, 1] }
    });
  });
