...
/* eslint-disable @nvidia-elements/lint/no-unexpected-slot-value -- Scene integration tests use direct render-content children intentionally. */
...
describe(SceneMarker.metadata.tag, () => {
  ...
  it('should define declarative marker attributes', async () => {
    ...
    `);
    const marker = fixture.querySelector<SceneMarker>(SceneMarker.metadata.tag);
  ...
  it('should give focusable markers a button role while preserving an authored role', async () => {
    ...
    `);
    const defaultMarker = fixture.querySelector<SceneMarker>('#default');
    const authoredMarker = fixture.querySelector<SceneMarker>('#authored');
  ...
  it('should match keyboard activation keys to button, link, and unrelated authored roles', async () => {
    ...
    `);
    const defaultMarker = fixture.querySelector<SceneMarker>('#default');
    const buttonMarker = fixture.querySelector<SceneMarker>('#button');
    const linkMarker = fixture.querySelector<SceneMarker>('#link');
    const statusMarker = fixture.querySelector<SceneMarker>('#status');
    await Promise.all([
