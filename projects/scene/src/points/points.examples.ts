...
export const Default = {
  render: () => html`
    ...
    <script type="module">
      ...
      writePoint(points, 0, { position: [0, 0, 0.2], color: [0, 1, 1, 1] });
      writePoint(points, 1, { position: [0.25, 0, 0.4], color: [1, 0, 1, 1] });
      writePoint(points, 2, { position: [0.5, 0, 0.6], color: [1, 1, 0, 1] });
...
export const ScatterPlot = {
  render: () => html`
    ...
    <script type="module">
      ...
      let seed = 2026;
      const count = 36;
      const points = new Uint8Array(count * POINT.stride);
...
export const SizeUnits = {
  render: () => html`
    ...
    <script type="module">
      ...
      const pixelPoints = new Uint8Array(POINT.stride);
      const worldPoints = new Uint8Array(POINT.stride);
      writePoint(pixelPoints, 0, { position: [-1.5, 0, 0.05], color: [0, 1, 1, 1] });
      writePoint(worldPoints, 0, { position: [1.5, 0, 0.05], color: [1, 0, 1, 1] });
...
export const Lidar = {
  render: () => html`
    ...
    <script type="module">
      ...
      const heightBytes = new Uint8Array(point * POINT.stride);
      const turbo = value => {
        const palette = [
          [0.18, 0.06, 0.28],
          [0.12, 0.38, 0.82],
      ...
      const intensityBytes = heightBytes.slice();
      for (let index = 0; index < point; index += 1) {
      ...
      document.querySelector('#lidar-color').addEventListener('click', event => {
        ...
        if (button) cloud.instances = button.dataset.color === 'intensity' ? intensityBytes : heightBytes;
      });
