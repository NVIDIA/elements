...
export const Default = {
  render: () => html`
    ...
    <script type="module">
      ...
      document.querySelector('#example-points').source = points;
    </script>
  `
};
...
export const ScatterPlot = {
  render: () => html`
    <nve-scene id="scatter-scene" aria-label="multicolor 3D scatter plot">
      <nve-scene-camera behavior="orbit" projection="perspective" target="[0,0,2.2]" distance="18" polar-angle="0.86" azimuth="-0.88" vertical-field-of-view="0.785398163"></nve-scene-camera>
...
export const SizeUnits = {
  render: () => html`
    ...
    <script type="module">
      ...
      document.querySelector('#pixel-size').source = pixelPoints;
      document.querySelector('#world-size').source = worldPoints;
    </script>
  `
};
  ...
  render: () => html`
    <div id="lidar-demo" style="position: relative; min-height: 100%;">
      <nve-scene id="lidar-scene" aria-label="Lidar street scan" style="width: 100%; height: 100%;">
        <nve-scene-camera id="lidar-camera-3d" behavior="orbit" target="[0,0,3.2]" distance="66" polar-angle="0.9" azimuth="-0.88" projection="perspective" vertical-field-of-view="0.785398163"></nve-scene-camera>
        <nve-scene-camera id="lidar-camera-plan" behavior="top" target="[0,0,0]" heading="-0.75" altitude="100" frustum-height="86" disabled></nve-scene-camera>
    ...
    <script type="module">
      ...
      const cameras = {
        '3d': scene.querySelector('#lidar-camera-3d'),
        plan: scene.querySelector('#lidar-camera-plan')
      };
      ...
      const applyCamera = view => {
        ...
        cameras[view].disabled = false;
      };
      const baseCount = Math.ceil(point / 2);
      let resolution = 2;
      const updateCount = () => {
        cloud.countLimit = Math.min(point, Math.round(baseCount * resolution));
        countOutput.value = cloud.countLimit.toLocaleString() + ' points';
      };
      cloud.source = heightPoints;
      applyCamera('3d');
      updateCount();
      ...
      document.querySelector('#lidar-color').addEventListener('click', event => {
        ...
        if (button) cloud.source = button.dataset.color === 'intensity' ? intensityPoints : heightPoints;
      });
      document.querySelector('#lidar-view').addEventListener('click', event => {
        ...
        if (button) applyCamera(button.dataset.view);
      });
