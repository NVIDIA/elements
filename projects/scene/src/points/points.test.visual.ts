...
describe('scene points visual runtime', () => {
  test('preserves sRGB point colors through the canvas output pipeline', async () => {
    const pixel = await visualRunner.inspect(
      ...
      `<nve-scene aria-label="sRGB point" style="width:512px;height:512px;background:rgb(0 0 0)"><nve-scene-points id="points" size="128" size-unit="pixel"></nve-scene-points></nve-scene><script type="module">
        ...
        document.querySelector('#points').instances = points;
      </script>`,
      page =>
        page.evaluate(async () => {
  ...
  test('draws the deterministic count prefix with alpha', async () => {
    const pixels = await visualRunner.inspect(
      ...
      `<nve-scene aria-label="points" style="width: 512px;height:512px;background:rgb(0 0 0)"><nve-scene-points id="points" size="64"></nve-scene-points></nve-scene><script type="module">
        ...
        points.add({ position: [0.8, 0, 0], color: [0, 1, 0, 1] });
      ...
      page =>
        page.evaluate(async () => {
          ...
          for (let index = 0; index < data.length; index += 4) {
            const red = data[index] as number;
            const green = data[index + 1] as number;
            if (red > 20 || green > 20) colored += 1;
          }
  ...
  test('projects world-sized points in scene units while preserving pixel-sized points', async () => {
    const bounds = await visualRunner.inspect(
      ...
      `<nve-scene aria-label="point size units" style="width:512px;height:512px;background:rgb(0 0 0)"><nve-scene-camera behavior="top" target="[0,0,0]" height="4"></nve-scene-camera><nve-scene-points id="world" size="1" size-unit="world"></nve-scene-points><nve-scene-points id="pixel" size="64"></nve-scene-points></nve-scene><script type="module">
        import { PointBuffer } from '@nvidia-elements/scene';
        ...
        const setPoint = (id, position, color) => {
          ...
          document.querySelector(id).instances = points;
        };
        ...
        page.evaluate(async () => {
          ...
          for (let y = 0; y < probe.height; y += 1) {
            for (let x = 0; x < probe.width; x += 1) {
              ...
              const key = red > green ? 'world' : green > 20 ? 'pixel' : undefined;
              if (key) {
    ...
    expect(bounds?.pixel).toBeGreaterThanOrEqual(63);
    expect(bounds?.pixel).toBeLessThanOrEqual(65);
    expect(bounds?.world).toBeGreaterThanOrEqual(127);
    expect(bounds?.world).toBeLessThanOrEqual(129);
