...
describe(Scene.metadata.tag, () => {
  ...
  it('should initialize its accessible shadow structure and resolve readiness after submit', async () => {
    ...
    const interactionSource = required(
      element.renderRoot.querySelector<HTMLElement>('[data-interaction-source]'),
      'Expected scene interaction source.'
    );
    ...
    expect(interactionSource).toBeInstanceOf(HTMLElement);
    expect(interactionSource.getAttribute('aria-hidden')).toBe('true');
    expect(interactionSource.hasAttribute('hidden')).toBe(true);
  ...
  it('restores native touch handling when the interactive camera is disabled', async () => {
    const gpu = configureFakeWebGPU();
    const { element } = await createScene(html`
      <nve-scene aria-label="Camera touch handling"><nve-scene-camera behavior="orbit"></nve-scene-camera></nve-scene>
    `);
    gpu.resolveNextDevice();
    await element.ready;
    const canvas = required(element.shadowRoot?.querySelector('canvas'), 'Expected scene canvas.');
    const camera = required(element.querySelector<SceneCamera>('nve-scene-camera'), 'Expected orbit camera.');
    expect(canvas.style.touchAction).toBe('none');
  ...
  it('moves the orbit camera toward world up on a downward left-button drag', async () => {
    const gpu = configureFakeWebGPU();
    const { element } = await createScene(html`
      <nve-scene aria-label="Vertical orbit"><nve-scene-camera behavior="orbit"></nve-scene-camera></nve-scene>
    `);
    gpu.resolveNextDevice();
    await element.ready;
    const canvas = required(element.shadowRoot?.querySelector('canvas'), 'Expected scene canvas.');
    const initialHeight = element.cameraState.pose.position[2];

    canvas.dispatchEvent(
      new PointerEvent('pointerdown', { bubbles: true, button: 0, buttons: 1, pointerId: 1, clientY: 10 })
    );
    canvas.dispatchEvent(
      new PointerEvent('pointermove', { bubbles: true, button: -1, buttons: 1, pointerId: 1, clientY: 30 })
    );
    await waitForAnimationFrames(2);

    expect(element.cameraState.pose.position[2]).toBeGreaterThan(initialHeight);
  });
  ...
  it('should retain the orbit pitch when a pointer drag would cross a pole', async () => {
    ...
    const initialHeight = element.cameraState.pose.position[2];
    canvas.dispatchEvent(
      new PointerEvent('pointerdown', { bubbles: true, button: 0, buttons: 1, pointerId: 1, clientY: 20 })
    );
    canvas.dispatchEvent(
      new PointerEvent('pointermove', { bubbles: true, button: -1, buttons: 1, pointerId: 1, clientY: 40 })
    );
    await waitForAnimationFrames(2);

    expect(element.cameraState.pose.position[2]).toBeCloseTo(initialHeight);
