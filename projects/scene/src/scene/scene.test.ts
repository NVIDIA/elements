...
import { type PickScope, type ScenePickRequest, type ScenePickResult } from '../internal/pick/routing.js';
...
describe(Scene.metadata.tag, () => {
  ...
  it('should wake once for a current frame transform and park again', async () => {
    ...
    Reflect.get(frame, 'setTransform').call(frame, { position: [1, 0, 0], orientation: [0, 0, 0, 1] });
  ...
  it('should order synthetic pointer results, dispatch a single routed click, and keep programmatic picks silent', async () => {
    const gpu = configureFakeWebGPU();
    const { element } = await createScene(html`
      <nve-scene aria-label="Pick scene"><nve-scene-cubes interactive><nve-scene-marker tabindex="0"></nve-scene-marker></nve-scene-cubes></nve-scene>
    `);
    ...
    let sceneClickEvents = 0;
    element.addEventListener('nve-scene-click', () => (sceneClickEvents += 1));
    ...
    expect(sceneClickEvents).toBe(0);
  });
  ...
  it('should stop canvas bubbles at the shadow boundary while routing one synthetic ancestor click', async () => {
    ...
    const { fixture, element } = await createScene(html`
      <div id="ancestor"><nve-scene aria-label="Pick scene"><nve-scene-cubes interactive><nve-scene-marker></nve-scene-marker></nve-scene-cubes></nve-scene></div>
    `);
  ...
  it('should bound a synthetic hover burst to one active resolver and the latest queued point', async () => {
    const gpu = configureFakeWebGPU();
    const { element } = await createScene(
      html`<nve-scene aria-label="Pick scene"><nve-scene-cubes interactive></nve-scene-cubes></nve-scene>`
    );
  ...
  it('should reject in-flight picks with the scene cycle error after disconnect or device loss', async () => {
    ...
    const pending: Array<{
      ...
      resolve: (hit: ScenePickResult | null) => void;
    }> = [];
    setScenePickDriverForTesting(element, () => new Promise((resolve, reject) => pending.push({ reject, resolve })));
    gpu.resolveNextDevice();
    await element.ready;
    ...
    await vi.waitFor(() => expect(pending).toHaveLength(1));
    element.remove();
    pending[0]?.reject(new Error('stale pick failure'));
  ...
  it('should opt automatic picking in per layer while preserving explicit full-scene picks', async () => {
    const gpu = configureFakeWebGPU();
    const { element } = await createScene(html`
      <nve-scene aria-label="Opt-in scene">
        <nve-scene-cubes><nve-scene-marker></nve-scene-marker></nve-scene-cubes>
      </nve-scene>
    `);
    const layer = element.querySelector<HTMLElement & { interactive: boolean }>('nve-scene-cubes');
    const marker = element.querySelector<HTMLElement>('nve-scene-marker');
    const canvas = element.shadowRoot?.querySelector('canvas');
    if (!layer || !marker || !canvas) throw new Error('Expected opt-in interaction targets.');
    vi.spyOn(canvas, 'getBoundingClientRect').mockReturnValue(new DOMRect(0, 0, 100, 100));
    const scopes: Array<PickScope | undefined> = [];
    setScenePickDriverForTesting(element, (_request, scope) => {
      scopes.push(scope);
      return Promise.resolve({ layer, marker, instanceIndex: 0, worldPosition: [1, 2, 3] });
    });
    gpu.resolveNextDevice();
    await element.ready;
    let unroutedClicks = 0;
    element.addEventListener('click', event => {
      if (event.target === element) unroutedClicks += 1;
    });
    ...
    expect(unroutedClicks).toBe(1);

    await expect(element.pick(10, 10)).resolves.toMatchObject({ element: marker, layer });
    expect(scopes).toEqual(['all']);

    layer.interactive = true;
    await elementIsStable(layer);
    canvas.dispatchEvent(new PointerEvent('click', { bubbles: true, composed: true, clientX: 10, clientY: 10 }));
    await vi.waitFor(() => expect(scopes).toEqual(['all', 'interactive']));
  ...
  it('should discard queued automatic results when the last interactive layer opts out', async () => {
    const gpu = configureFakeWebGPU();
    const { element } = await createScene(html`
      <nve-scene aria-label="Changing interaction scene">
        <nve-scene-cubes interactive><nve-scene-marker></nve-scene-marker></nve-scene-cubes>
      </nve-scene>
    `);
    const layer = element.querySelector<HTMLElement & { interactive: boolean }>('nve-scene-cubes');
    const marker = element.querySelector<HTMLElement>('nve-scene-marker');
    const canvas = element.shadowRoot?.querySelector('canvas');
    if (!layer || !marker || !canvas) throw new Error('Expected changing interaction targets.');
    vi.spyOn(canvas, 'getBoundingClientRect').mockReturnValue(new DOMRect(0, 0, 100, 100));
    let resolvePick!: (hit: ScenePickResult | null) => void;
    setScenePickDriverForTesting(element, () => new Promise(resolve => (resolvePick = resolve)));
    gpu.resolveNextDevice();
    await element.ready;
    const enter = vi.fn();
    ...
    await elementIsStable(layer);
    await waitForAnimationFrames(2);
    resolvePick({ layer, marker, instanceIndex: 0, worldPosition: [0, 0, 0] });
    await Promise.resolve();

    expect(enter).not.toHaveBeenCalled();
  });

  it('should route buffer clicks and misses exactly once, including the resolved hit detail', async () => {
    const gpu = configureFakeWebGPU();
    const { element } = await createScene(
      html`<nve-scene aria-label="Pick scene"><nve-scene-cubes interactive></nve-scene-cubes></nve-scene>`
    );
    ...
    layer.addEventListener('nve-scene-click', event => {
      events.push('scene-click');
    ...
    pending[1]?.(null);
    await vi.waitFor(() => expect(events).toEqual(['layer', 'scene-click', 'scene']));
  ...
  it('should dispatch marker hover crossings and pointer metadata, and ignore rejected picks', async () => {
    ...
    const { element } = await createScene(html`
      <nve-scene aria-label="Hover scene">
        <nve-scene-cubes interactive><nve-scene-marker tabindex="0"></nve-scene-marker></nve-scene-cubes>
      </nve-scene>
    `);
    const layer = element.querySelector<HTMLElement>('nve-scene-cubes');
    const marker = element.querySelector<HTMLElement>('nve-scene-marker');
    ...
    layer.addEventListener('nve-scene-pointerleave', () => events.push('layer-leave'));
    ...
    await vi.waitFor(() => expect(pending).toHaveLength(2));
    pending[1]?.resolve({ layer, marker, instanceIndex: 0, worldPosition: [0, 0, 0] });
    await Promise.resolve();
    expect(events).toEqual(['enter:9:4:1']);

    canvas.dispatchEvent(
      new PointerEvent('pointermove', { bubbles: true, composed: true, clientX: 8, clientY: 9, pointerId: 11 })
    );
    await vi.waitFor(() => expect(pending).toHaveLength(3));
    pending[2]?.resolve({ layer, instanceIndex: 0, worldPosition: [0, 0, 0] });
    await vi.waitFor(() => expect(events).toEqual(['enter:9:4:1', 'leave']));

    canvas.dispatchEvent(new PointerEvent('pointermove', { bubbles: true, composed: true, clientX: 10, clientY: 10 }));
    await vi.waitFor(() => expect(pending).toHaveLength(4));
    pending[3]?.resolve(null);
    await vi.waitFor(() => expect(events).toEqual(['enter:9:4:1', 'leave', 'layer-leave']));

    canvas.dispatchEvent(new PointerEvent('pointermove', { bubbles: true, composed: true, clientX: 11, clientY: 11 }));
    await vi.waitFor(() => expect(pending).toHaveLength(5));
    pending[4]?.reject(new Error('readback failed'));
    await Promise.resolve();
    expect(events).toEqual(['enter:9:4:1', 'leave', 'layer-leave']);
  });
  ...
  it('should resolve follow camera state for host-updated named frame poses without a change event', async () => {
    ...
    const frame = element.querySelector<HTMLElement>('nve-scene-frame');
    if (!frame) throw new Error('Expected frame.');
    Reflect.get(frame, 'setTransform').call(frame, { position: [2, 3, 4], orientation: [0, 0, 0, 1] });
    let changes = 0;
    ...
    Reflect.get(frame, 'setTransform').call(frame, { position: [5, 6, 7], orientation: [0, 0, 1, 0] });
    await waitForAnimationFrames(2);
    expect(element.cameraState.pose.position).toEqual([
      expect.closeTo(5),
  ...
  it('should compose a direct pose with one uniquely valid named frame and recover from duplicates', async () => {
    ...
    map.setTransform({ position: [8, 0, 0], orientation: [0, 0, 0, 1] });
    await waitForAnimationFrames(2);
  ...
  it('should make conflicting, invalid, and disabled camera behaviors inert until they recover', async () => {
    ...
    const recovered = first.theta;
    element.dispatchEvent(new KeyboardEvent('keydown', { bubbles: true, key: 'ArrowRight' }));
    await waitForAnimationFrames(2);
    expect(first.theta).toBeCloseTo(recovered + Math.PI / 36);
  ...
  it('should keep unresolved, duplicate, and invalid pose follows inert and recover when the frame becomes valid', async () => {
    const gpu = configureFakeWebGPU();
    const { element } = await createScene(html`
      <nve-scene aria-label="Follow resolution"><nve-scene-camera behavior="follow" frame="robot" mode="pose"></nve-scene-camera></nve-scene>
    `);
    const errors: string[] = [];
    element.addEventListener('nve-scene-error', event =>
      errors.push((event as CustomEvent<SceneErrorDetail>).detail.code)
    ...
    second.remove();
    Reflect.get(first, 'setTransform').call(first, { position: [3, 0, 0], orientation: [0, 0, 0, 1] });
    await waitForAnimationFrames(2);
  ...
  it('should apply camera keyboard, pointer, wheel, and touch sources without writing public inputs', async () => {
    ...
    const initialTheta = camera.theta;
    element.dispatchEvent(new KeyboardEvent('keydown', { bubbles: true, key: 'ArrowRight' }));
    await waitForAnimationFrames(2);
    expect(camera.theta).toBeCloseTo(initialTheta + Math.PI / 36);
    canvas.dispatchEvent(new WheelEvent('wheel', { bubbles: true, cancelable: true, deltaY: 100 }));
    await waitForAnimationFrames(2);
    ...
    await waitForAnimationFrames(2);
    expect(camera.theta).toBeCloseTo(initialTheta + Math.PI / 36 - 0.05);
  ...
  it('should retain the orbit pitch when a pointer drag would cross a pole', async () => {
    ...
    const { element } = await createScene(html`
      <nve-scene aria-label="Camera pole limit">
        <nve-scene-camera behavior="orbit" phi="0.05"></nve-scene-camera>
      </nve-scene>
    `);
    gpu.resolveNextDevice();
    await element.ready;
  ...
  it('should pan the orbit camera with a right-button drag and suppress the canvas context menu', async () => {
    ...
    const scale = (2 * initial.distance * Math.tan(camera.projection === 'perspective' ? camera.fovy / 2 : 0.5)) / 100;
    const source: string[] = [];
  ...
  it('should preserve a followed target for Shift+Arrow and leave unhandled keys alone', async () => {
    ...
    Reflect.get(frame, 'setTransform').call(frame, { position: [3, 4, 0], orientation: [0, 0, 0, 1] });
    gpu.resolveNextDevice();
    await element.ready;
    const before = { distance: orbit.distance, phi: orbit.phi };
    ...
    expect(orbit.phi).toBeCloseTo(before.phi - Math.PI / 36);
  ...
  it('should resolve authoritative camera properties and preserve user changes across frames', async () => {
    ...
    gpu.resolveNextDevice();
    await element.ready;
    expect(camera).toMatchObject({ distance: 20, phi: 0.5, theta: 0 });
    expect(element.cameraState).toMatchObject({
    ...
    camera.distance = 6;
    await waitForAnimationFrames(2);
    expect(camera).toMatchObject({ distance: 6, phi: 0.5, theta: Math.PI / 36 });
  ...
  it('should retain direct valid labels in overlay slots and recover every label contract', async () => {
    ...
    frame.setAttribute('position', '[0,0]');
    await waitForAnimationFrames(2);
    expect(errors.filter(event => event.detail.code === 'label-frame-unresolved')).toHaveLength(2);
    expect(overlay?.querySelector('slot')).toBeNull();
  ...
  it('should isolate nested labels while preserving their accessibility DOM and direct-slot ownership', async () => {
    ...
    expect(outer.shadowRoot?.querySelector<HTMLSlotElement>('.overlay slot')?.assignedElements()).toEqual([outerLabel]);
    expect(inner.shadowRoot?.querySelector<HTMLSlotElement>('.overlay slot')?.assignedElements()).toEqual([innerLabel]);
  ...
  it('should recapture promoted labels for label-content attribute and text mutations', async () => {
    ...
    gpu.resolveNextDevice();
    await element.ready;
    notifyMutation(mutationCallbacks[0], [createChildListMutation(element, [label])]);
    await vi.waitFor(() => expect(copies.length).toBeGreaterThan(0));
  ...
  it('should recapture a promoted label after internal child content childList mutations', async () => {
    ...
    child.append(detail);
    notifyMutation(mutationCallbacks[0], [createChildListMutation(child, [detail])]);
  ...
  it('prefetches texture label pixels, favors an unavailable cache, and routes only geometry-in-front input', async () => {
    const gpu = configureFakeWebGPU();
    const { element } = await createScene(html`
      <nve-scene aria-label="Texture pointer"><nve-scene-cubes interactive><nve-scene-marker></nve-scene-marker></nve-scene-cubes></nve-scene>
    `);
    const label = createLabel();
...
function createLabel(options: { frame?: string } = {}): SceneLabel {
  const label = document.createElement(SceneLabel.metadata.tag) as SceneLabel;
...
function setStreamBytes(layer: HTMLElement, count: number, property: 'instances' | 'vertices'): void {
  const layout =
