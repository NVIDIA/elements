...
/**
 * @summary Replacement, full publication, and ranged publication modes update identical packed marker data. Use this diagnostic to separate producer work from Scene ingestion and identify when small ranged updates reduce update cost.
 */
    ...
    <script type="module">
      ...
      async function allocate() {
        ...
        if (sourceMode === 'versioned') {
          ...
          const bytes = markerBuffer.mutableBytes;
          const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
          const positionOffset = MARKER.fields.position.offset;
          const colorOffset = MARKER.fields.color.offset;
          for (let index = 0; index < recordCount; index += 1) {
