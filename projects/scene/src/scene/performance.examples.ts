...
/**
 * @summary Static, animated cube loads isolate main-thread frame cadence from record generation and streaming updates. Use this diagnostic to compare opaque and translucent rendering at controlled instance counts.
 */
  ...
  render: () => html`
    <section id="scene-render-load" nve-layout="column gap:sm">
      ...
      <nve-scene aria-label="static render load" style="display:block;height:420px;">
        <nve-scene-camera behavior="orbit" target="[0,0,0]" distance="95" polar-angle="1.1"></nve-scene-camera>
    ...
    <script type="module">
      ...
      const cubeLayer = root.querySelector('nve-scene-cubes');
      ...
      function rebuild() {
        ...
        const cubes = new CubeBuffer({ capacity: count });
        const bytes = cubes.mutableBytes;
...
/**
 * @summary Replacement, full publication, and ranged publication modes update identical packed cube data. Use this diagnostic to separate producer work from Scene ingestion and identify when small ranged updates reduce update cost.
 */
  ...
  render: () => html`
    <section id="scene-update-strategy" nve-layout="column gap:sm">
      <div nve-layout="row gap:sm align:wrap">
        <nve-select layout="vertical-inline" fit-content>
          ...
          <select id="scene-update-strategy-mode">
            ...
            <option value="full">publish all</option>
            <option value="ranged" selected>publish 1%</option>
          </select>
        </nve-select>
        <nve-select layout="vertical-inline" fit-content>
      ...
      <nve-scene aria-label="streaming update strategy" style="display:block;height:420px;">
        <nve-scene-camera behavior="orbit" target="[0,0,1]" distance="95" polar-angle="1.1"></nve-scene-camera>
        <nve-scene-cubes></nve-scene-cubes>
      </nve-scene>
      <output nve-text="code">paused</output>
    </section>
    <script type="module">
      ...
      function update(time) {
        ...
        if (mode === 'replace') {
          active = active === 0 ? 1 : 0;
          mutate(buffers[active].mutableBytes, 0, count, time * 0.03);
          cubeLayer.source = buffers[active];
        } else {
          const source = buffers[0];
          const start = mode === 'ranged' ? rangeStart : 0;
          mutate(source.mutableBytes, start, updateCount, time * 0.03);
          cubeLayer.publish({ start, count: updateCount });
  ...
  render: () => html`
    ...
    <script type="module">
      ...
      const workloads = {
        ...
        triangles: { cadence: 50, count: 30000, stride: TRIANGLE_VERTEX.stride, buffers: [], active: 0, layers: [], tick: 0, next: 0 },
      ...
      function createScene(index) {
        ...
        camera.setAttribute('polar-angle', '1.1');
        scene.append(camera);
      ...
      function fillTriangles(bytes, tick) {
        const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
        const positionOffset = TRIANGLE_VERTEX.fields.position.offset;
        const colorOffset = TRIANGLE_VERTEX.fields.color.offset;
      ...
      function fillInstances(bytes, tick, kind) {
        const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
        const positionOffset = MARKER.fields.position.offset;
        const scaleOffset = MARKER.fields.scale.offset;
        const colorOffset = MARKER.fields.color.offset;
      ...
      function fillWorkload(name, buffer, tick) {
        const bytes = buffer.mutableBytes;
        if (name === 'lines') fillLines(bytes, tick);
        else if (name === 'triangles') fillTriangles(bytes, tick);
        else fillInstances(bytes, tick, name);
        buffer.setCount(workloads[name].count);
      }
      ...
      function createBuffers(name) {
        ...
        for (const layer of workload.layers) {
          if (name === 'lines' || name === 'triangles') layer.source = workload.buffers[0];
          else layer.source = workload.buffers[0];
        }
      }
      ...
      function updateWorkload(name, time) {
        ...
        for (const layer of workload.layers) {
          if (name === 'lines' || name === 'triangles') layer.source = buffer;
          else layer.source = buffer;
        }
  ...
  render: () => html`
    ...
    <script type="module">
      ...
      async function allocate() {
        ...
        const cubes = new CubeBuffer({ capacity: recordCount });
        if (sourceMode === 'versioned') {
          for (let index = 0; index < recordCount; index += 1) {
            cubes.set(index, {
              color: [118 / 255, 185 / 255, 0, 1],
              position: [index % 500, Math.floor(index / 500), 0]
            });
          }
          producer = cubes;
        } else {
