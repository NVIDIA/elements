...
export const Default = {
  render: () => html`
    ...
    <script type="module">
      ...
      const vertices = new Uint8Array(3 * TRI_VERTEX.stride);
      writeTriVertex(vertices, 0, { position: [-0.75, -0.5, 0.05], color: [0, 1, 1, 1, 0.7] });
      writeTriVertex(vertices, 1, { position: [0.75, -0.5, 0.05], color: [1, 0, 1, 1, 0.7] });
      writeTriVertex(vertices, 2, { position: [0, 1, 0.05], color: [1, 1, 0, 1, 0.7] });
...
export const Path = {
  render: () => html`
    ...
    <script type="module">
      ...
      const bytes = new Uint8Array(vertices.length * TRI_VERTEX.stride);
      vertices.forEach((vertex, index) => writeTriVertex(bytes, index, vertex));
      corridor.vertices = bytes;
