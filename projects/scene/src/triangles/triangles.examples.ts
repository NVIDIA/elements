...
export const Default = {
  render: () => html`
    ...
    <script type="module">
      ...
      document.querySelector('#example-triangles').source = vertices;
    </script>
  `
};

/**
 * @summary An unlit, per-vertex-color driving corridor streams as triangle-list records. Use ranged publication for transient planning or perception overlays that change frequently.
 */
export const Path = {
  render: () => html`
    ...
    <script type="module">
      ...
      corridor.source = buffer;

      // On a later planning cycle, mutate affected records and call corridor.publish({ start, count }).
    </script>
  `
};
