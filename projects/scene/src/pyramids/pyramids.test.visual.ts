...
describe('scene pyramids visual runtime', () => {
  test('should render flat square-pyramid faces and pick its marker through the real WebGPU profile', async () => {
    const result = await webgpuVisualRunner.inspect(
      ...
      /* html */ `
        <nve-scene aria-label="Square pyramid scene" style="width: 192px; height: 144px; background: rgb(0 0 0)">
          <nve-scene-camera behavior="orbit" target="[0,0,0]" heading="0" distance="8" phi="0.9" theta="-0.75" projection="ortho" frustum-height="3.8"></nve-scene-camera>
