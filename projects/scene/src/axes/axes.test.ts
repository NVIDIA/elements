...
describe(SceneAxes.metadata.tag, () => {
  ...
  it('reflects and normalizes direction while regenerating signed axis vertices', async () => {
    ...
    expect(axes.direction).toBe('bidirectional');
    expect(axes.getAttribute('direction')).toBe('bidirectional');
    expect(readLineVertex(takeStreamingLayerRenderData(axes).bytes!, 0).position).toEqual([-2, 0, 0]);
    expect(readLineVertex(takeStreamingLayerRenderData(axes).bytes!, 5).position).toEqual([0, 0, 2]);
