...
describe('scene axes visual runtime', () => {
  test('renders the fixed RGB basis at the independently derived asymmetric-camera positions', async () => {
    ...
    // Hand-derived for an orthographic 512 x 512 viewport, polar-angle=pi/3 and azimuth=-pi/3.
...
function sceneTemplate(layers: string): string {
  return /* html */ `
    <nve-scene aria-label="Axes orientation scene" style="width: 512px; height: 512px; background: rgb(0 0 0)">
      <nve-scene-camera behavior="orbit" target="[0,0,0]" heading="0" distance="12" polar-angle="${Math.PI / 3}" azimuth="${-Math.PI / 3}" projection="orthographic" frustum-height="8"></nve-scene-camera>
      ${layers}
    </nve-scene>
    <script type="module">
