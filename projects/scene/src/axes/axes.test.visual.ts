...
describe('scene axes visual runtime', () => {
  test('renders the fixed RGB basis at the independently derived asymmetric-camera positions', async () => {
    const tips = await webgpuVisualRunner.inspect('scene-axes-orientation-t2', orientationTemplate(), page =>
      page.evaluate(async () => {
        ...
        return {
          blue: locateTipCluster(pixels, probe.width, { color: 'blue', x: 100, y: 53 }),
          green: locateTipCluster(pixels, probe.width, { color: 'green', x: 127, y: 77 }),
          red: locateTipCluster(pixels, probe.width, { color: 'red', x: 154, y: 116 })
        };
    ...
    // Hand-derived for an orthographic 200 x 200 viewport, phi=pi/3 and theta=-pi/3.
    // The view basis is right=(sqrt(3)/2, 1/2, 0) and up=(-1/4, sqrt(3)/4, sqrt(3)/2).
    // With an 8-unit frustum, a point p maps to (100 + 25 * dot(right,p),
    // 100 - 25 * dot(up,p)); tips at 2.5 units are +X=(154,116), +Y=(131,73),
    // +Z=(100,46). These values deliberately do not import or invoke production camera
    // math, so coordinate-system regressions cannot update the expected answer with it.
    expect(tips.red.count).toBeGreaterThan(8);
    expect(Math.abs(tips.red.x - 154)).toBeLessThanOrEqual(7);
    expect(Math.abs(tips.red.y - 116)).toBeLessThanOrEqual(7);
    expect(tips.green.count).toBeGreaterThan(8);
    expect(Math.abs(tips.green.x - 131)).toBeLessThanOrEqual(7);
    expect(Math.abs(tips.green.y - 73)).toBeLessThanOrEqual(7);
    expect(tips.blue.count).toBeGreaterThan(8);
    expect(Math.abs(tips.blue.x - 100)).toBeLessThanOrEqual(7);
    expect(Math.abs(tips.blue.y - 46)).toBeLessThanOrEqual(7);
  });

  test('keeps axes and grids transparent to the WebGPU ID pass and pick events', async () => {
    const result = await webgpuVisualRunner.inspect('scene-axes-pick-transparency-t2', pickTemplate(), page =>
      page.evaluate(async () => {
        const scene = document.querySelector('nve-scene');
        ...
        let axesPickEvents = 0;
        let gridPickEvents = 0;
        axes.addEventListener('nve-scene-pick', () => (axesPickEvents += 1));
        grid.addEventListener('nve-scene-pick', () => (gridPickEvents += 1));
    ...
    expect(result.axesPickEvents).toBe(0);
    expect(result.gridPickEvents).toBe(0);
...
function sceneTemplate(layers: string): string {
  return /* html */ `
    <nve-scene aria-label="Axes orientation scene" style="width: 200px; height: 200px; background: rgb(0 0 0)">
