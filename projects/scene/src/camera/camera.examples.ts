...
export const Default = {
  render: () => html`
    <nve-page id="pose-controls-demo">
      <main>
        <nve-scene id="pose-controls-scene" aria-label="interactive optical pose camera" style="width: 100%; height: 100%;">
          <nve-scene-camera behavior="pose" position="[0,-8.5,8.5]" orientation="[-0.92388,0,0,0.382683]" near="0.5" far="40" vertical-field-of-view="0.872665"></nve-scene-camera>
            ...
            <div nve-layout="column gap:sm" role="group" aria-labelledby="pose-controls-projection-heading">
              ...
              <div nve-layout="grid gap:sm span-items:6">
                ...
                <div id="pose-controls-perspective">
                  <nve-range>
                    <label for="pose-controls-verticalFieldOfView">Field of view <output for="pose-controls-verticalFieldOfView">50</output>°</label>
                    <input id="pose-controls-verticalFieldOfView" name="verticalFieldOfView" type="range" min="30" max="100" step="1" value="50" />
                  </nve-range>
                </div>
                <div id="pose-controls-orthographic" hidden>
                  <nve-range>
    ...
    <script type="module">
      ...
      const orthographic = demo.querySelector('#pose-controls-orthographic');
      ...
      const update = () => {
        ...
        camera.verticalFieldOfView = radians(number('verticalFieldOfView'));
        ...
        orthographic.hidden = projection.value !== 'orthographic';
  ...
  render: () => html`
      <div style="position: relative;">
        <nve-scene aria-label="default camera viewing a rover" style="min-height: 480px">
          <nve-scene-gridlines count="12" spacing="2"></nve-scene-gridlines>
          <nve-scene-camera behavior="orbit" target="[0,0,1]" distance="8" polar-angle="0.9" azimuth="-0.75"></nve-scene-camera>
...
export const BehaviorFollow = {
  render: () => html`
    <nve-scene id="camera-follow-scene" aria-label="follow camera tracking a rover on a circular course" style="min-height: 480px">
      <nve-scene-camera behavior="follow" frame="camera-follow-rover" follow-mode="pose"></nve-scene-camera>
      <nve-scene-camera behavior="orbit" projection="perspective" target="[0,0,1]" distance="11" polar-angle="1.18" azimuth="3.141592654" vertical-field-of-view="0.785398163"></nve-scene-camera>
      <nve-scene-gridlines count="12" spacing="2"></nve-scene-gridlines>
...
export const BehaviorTop = {
  render: () => html`
    <nve-scene id="camera-top-scene" aria-label="top camera viewing a rover on a circular course" style="min-height: 480px">
      <nve-scene-camera behavior="top" target="[0,0,0]" altitude="60" frustum-height="34"></nve-scene-camera>
      <nve-scene-gridlines count="12" spacing="2"></nve-scene-gridlines>
