...
describe('camera visual contract', () => {
  ...
  test('applies orthographic top-camera projection before rendering', async () => {
    const state = await inspectCameraState(
      'scene-camera-top',
      '<nve-scene-camera behavior="top" altitude="30" frustum-height="30"></nve-scene-camera>'
    );
    expect(state.pose.position).toEqual([0, 0, 30]);
    expect(state.projection).toEqual({ mode: 'orthographic', frustumHeight: 30, near: 0.01, far: 10_000 });
  });
