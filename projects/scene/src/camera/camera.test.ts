...
import {
  SCENE_CAMERA_PROPERTY_BEHAVIORS,
  SceneCamera,
  sceneCameraController,
  type SceneCameraBehavior
} from './camera.js';
import './define.js';
...
describe(SceneCamera.metadata.tag, () => {
  ...
  it('defaults to a static pose behavior', async () => {
    ...
    expect(sceneCameraController.getContribution(camera)).toEqual({
      ...
      projection: { mode: 'perspective', verticalFieldOfView: Math.PI / 4, near: 0.01, far: 10_000 }
    });
    expect(camera.behavior).toBe('pose');
    expect('applyCameraRuntimeWrite' in camera).toBe(false);
    expect('cameraInputRevision' in camera).toBe(false);
    expect('isCameraInputExplicit' in camera).toBe(false);
    expect(Object.isFrozen(SCENE_CAMERA_PROPERTY_BEHAVIORS)).toBe(true);
    expect(Object.values(SCENE_CAMERA_PROPERTY_BEHAVIORS).every(Object.isFrozen)).toBe(true);
  ...
  it('validates the orbit pose and limits and restores defaults after attributes are removed', async () => {
    ...
    camera.setAttribute('polar-angle', '0.5');
    camera.setAttribute('azimuth', '1');
    ...
    expect(sceneCameraController.getContribution(camera)).toBeNull();
    camera.distance = 12;
    camera.polarAngle = Math.PI + 1;
    expect(sceneCameraController.getContribution(camera)).toBeNull();
    camera.polarAngle = Math.PI / 4;
    camera.azimuth = Number.NaN;
    expect(sceneCameraController.getContribution(camera)).toBeNull();

    camera.removeAttribute('distance');
    camera.removeAttribute('polar-angle');
    camera.removeAttribute('azimuth');
  ...
  it('validates authoritative orbit target and projection properties', async () => {
    ...
    expect(sceneCameraController.getContribution(camera)).toMatchObject({
      projection: { mode: 'orthographic', frustumHeight: 8, near: 0.01, far: 10_000 },
  ...
  it('validates follow configuration and frame resolution', async () => {
    ...
    camera.frame = 'robot';
    camera.setAttribute('follow-mode', 'unsupported');
    await elementIsStable(camera);
    expect(camera.followMode).toBe('position');
    expect(sceneCameraController.getContribution(camera)?.fields).toEqual(['target.position']);
    camera.setAttribute('follow-mode', 'pose');
    await elementIsStable(camera);
    expect(camera.followMode).toBe('pose');
    camera.removeAttribute('follow-mode');
    await elementIsStable(camera);
    expect(camera.followMode).toBe('position');
  ...
  it('validates top altitude and recovers by property', async () => {
    const camera = await createCamera('top');
    const errors: CustomEvent[] = [];
    vi.spyOn(console, 'error').mockImplementation(() => undefined);
    camera.addEventListener('nve-scene-error', event => errors.push(event as CustomEvent));
    camera.altitude = Number.NaN;
    await elementIsStable(camera);
    expect(sceneCameraController.getContribution(camera)).toBeNull();
    ...
    camera.altitude = 12;
    await elementIsStable(camera);
    expect(sceneCameraController.getContribution(camera)).toMatchObject({ altitude: 12, kind: 'top' });
  });

  it('separates top altitude and orthographic extent', async () => {
    const camera = await createCamera('top');
    camera.altitude = 30;
    camera.frustumHeight = 24;
    expect(sceneCameraController.getContribution(camera)).toMatchObject({
      altitude: 30,
      projection: { frustumHeight: 24, mode: 'orthographic' }
    });

    camera.setAttribute('altitude', '80');
    camera.setAttribute('frustum-height', '12');
    await elementIsStable(camera);
    expect(sceneCameraController.getContribution(camera)).toMatchObject({
      altitude: 80,
      projection: { frustumHeight: 12, mode: 'orthographic' }
    });

    camera.removeAttribute('altitude');
    camera.removeAttribute('frustum-height');
    await elementIsStable(camera);
    expect(sceneCameraController.getContribution(camera)).toMatchObject({
      altitude: 40,
      projection: { frustumHeight: 40, mode: 'orthographic' }
    });
  ...
  it('tracks JavaScript assignments equal to defaults as explicit author intent', async () => {
    const camera = await createCamera('follow');
    const errors: CustomEvent[] = [];
    ...
    sceneCameraController.getContribution(camera);
    expect(errors).toHaveLength(1);
    expect(errors[0]?.detail).toMatchObject({ code: 'camera-property-inactive', severity: 'warning' });
  });
