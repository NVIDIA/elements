...
export const Default = {
  render: () => html`
    <nve-scene aria-label="Labeled cubes">
      ...
      <nve-scene-cubes>
        <nve-scene-marker position="[-2,0,0.75]" scale="[1.5,1.5,1.5]" color="cyan"></nve-scene-marker>
        <nve-scene-marker position="[2,0,1]" scale="[2,2,2]" color="magenta"></nve-scene-marker>
      </nve-scene-cubes>
...
export const CssState = {
  render: () => html`
    <style>
      nve-scene-label[stale] .callout { color: orange; }
      nve-scene-label[occluded] .callout { color: red; }
    </style>
