...
describe(SceneHeightfield.metadata.tag, () => {
  ...
  it('provides a normalized color and a property-only grid without reflection', async () => {
    ...
    await elementIsStable(heightfield);
    expect(heightfield.interactive).toBe(true);
    heightfield.interactive = true;
    expect(heightfield.interactive).toBe(true);
  ...
  it('multiplies parsed base color with samples and treats alpha as transparent', () => {
    ...
    const version = getHeightfieldLayerVersion(heightfield);
    heightfield.color = '#123456';
    expect(getHeightfieldLayerVersion(heightfield)).toBe(version);
    heightfield.grid = validGrid({
  ...
  it('uses owned terrain for local queries and drape semantics', () => {
    ...
    expect(heightfield.surfaceHeightAt(0.5, 0.5)).toBe(1.5);
    expect(heightfield.surfaceNormalAt(0.5, 0.5)?.[2]).toBeGreaterThan(0);
    expect(heightfield.surfaceSlopeAt(0.5, 0.5)).toBeGreaterThan(0);
    expect(heightfield.drapeToSurface(points, 1)).toEqual(new Float32Array([0, 0, 1, 4, 4, 7]));
