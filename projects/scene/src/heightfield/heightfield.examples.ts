...
export const Default = {
  render: () => html`
    <nve-scene aria-label="Colorful heightfield scene">
      <nve-scene-camera behavior="orbit" target="[0,0,0.5]" distance="8" polar-angle="1" azimuth="-0.7"></nve-scene-camera>
  ...
  render: () => html`
    <section nve-layout="column gap:sm">
      <nve-scene aria-label="Mobile robot warehouse floor survey">
        <nve-scene-camera behavior="orbit" target="[0,-1,0.22]" distance="13" polar-angle="0.95" azimuth="-2"></nve-scene-camera>
        <nve-scene-gridlines></nve-scene-gridlines>
    ...
    <script type="module">
      ...
      const drapedRoute = terrain.drapeToSurface(routePoints, 0.055);
...
export const TerrainExplorer = {
  render: () => html`
    ...
    <script type="module">
      ...
      const drapedRoute = terrain.drapeToSurface(routePoints, 0.15);
