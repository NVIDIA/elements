...
import { HEIGHTFIELD_COMPUTE_SHADER } from '../internal/heightfield/gpu.js';
...
describe('scene heightfield visual runtime', () => {
  test('renders a smooth, colored terrain, updates it without rebuilding topology, and picks its surface', async () => {
    const result = await visualRunner.inspect('scene-heightfield-h2', template(), page =>
      page.evaluate(
        async shaders => {
          await validateShader(shaders.heightfield);
          const { canvas, scene, terrain } = getFixtureElements();
          ...
          const analyze = (image: { data: Uint8ClampedArray; height: number; width: number }) => {
            const { active, blue, green, red } = collectActivePixels(image);
            ...
            const { axialDeltas, diagonalDeltas, luminance } = collectPixelDeltas(active);
            ...
            const pickPixel = findInteriorPixels(active, luminance).sort(
              (left, right) =>
          ...
          const grid = (
            globalThis as typeof globalThis & { terrainGrid: { heights: Float32Array; colors: Uint8Array } }
          ).terrainGrid;
          updateGrid(grid);
          ...
          const bilinear = world
            ? (terrain as unknown as { heightAt(x: number, y: number): number | undefined }).heightAt(
                world[0],
                world[1]
              )
            : undefined;
          ...
          return {
            ...
            hit: hit
              ? {
                  ...
                  marker: hit.element === hit.layer ? null : hit.element.id,
          ...
          async function validateShader(code: string): Promise<void> {
            const gpu = navigator.gpu;
            if (!gpu) throw new Error('Expected WebGPU support.');
            const adapter = await gpu.requestAdapter();
            if (!adapter) throw new Error('Expected a WebGPU adapter.');
            const device = await adapter.requestDevice();
            const information = await device.createShaderModule({ code }).getCompilationInfo();
            device.destroy();
          ...
          function getFixtureElements(): {
            readonly canvas: HTMLCanvasElement;
            readonly scene: HTMLElement;
            readonly terrain: HTMLElement;
          } {
            const fixtureScene = document.querySelector('nve-scene');
            const fixtureTerrain = document.querySelector('#terrain');
            const fixtureCanvas = fixtureScene?.shadowRoot?.querySelector('canvas');
            if (
              !(fixtureScene instanceof HTMLElement) ||
              !(fixtureTerrain instanceof HTMLElement) ||
              !(fixtureCanvas instanceof HTMLCanvasElement)
            ) {
              throw new Error('Expected the heightfield scene, terrain, and canvas.');
            }
            return { canvas: fixtureCanvas, scene: fixtureScene, terrain: fixtureTerrain };
          }

          function collectActivePixels(image: { data: Uint8ClampedArray; height: number; width: number }) {
            ...
            for (let y = 0; y < image.height; y += 1) {
              for (let x = 0; x < image.width; x += 1) {
                ...
                active.push({ x, y, luminance: 0.2126 * red + 0.7152 * green + 0.0722 * blue });
          ...
          function classifyColor(red: number, green: number, blue: number): 'blue' | 'green' | 'red' | undefined {
            if (red > green + 18 && red > blue + 18) return 'red';
            if (green > red + 12 && green > blue + 12) return 'green';
            return blue > red + 12 && blue > green + 12 ? 'blue' : undefined;
          }

          function collectPixelDeltas(active: Array<{ x: number; y: number; luminance: number }>) {
            ...
            return { axialDeltas, diagonalDeltas, luminance };
          }

          function findInteriorPixels(
            active: Array<{ x: number; y: number; luminance: number }>,
            luminance: ReadonlyMap<string, number>
          ) {
            return active.filter(pixel =>
              [
                `${pixel.x - 1},${pixel.y}`,
                `${pixel.x + 1},${pixel.y}`,
                `${pixel.x},${pixel.y - 1}`,
                `${pixel.x},${pixel.y + 1}`
              ].every(neighbor => luminance.has(neighbor))
            );
          }

          function updateGrid(targetGrid: { heights: Float32Array; colors: Uint8Array }): void {
            for (let index = 0; index < targetGrid.heights.length; index += 1) {
              targetGrid.heights[index] = (targetGrid.heights[index] ?? 0) * -0.65 + 0.5;
              const offset = index * 4;
              targetGrid.colors[offset] = 20;
              targetGrid.colors[offset + 1] = 80 + ((index * 23) % 150);
              targetGrid.colors[offset + 2] = 255 - ((index * 11) % 140);
            }
          }
        },
        { heightfield: HEIGHTFIELD_COMPUTE_SHADER }
      )
    );
    ...
    expect(result.hit).toMatchObject({ layerId: 'terrain', marker: null, targetKind: 'surface' });
...
function template(): string {
  return /* html */ `
    <nve-scene aria-label="Heightfield visual test" style="width: 512px; height: 512px; background: rgb(0 0 0)">
      <nve-scene-camera behavior="orbit" target="[0,0,0]" heading="0" distance="10" polar-angle="0.8" azimuth="0.65" projection="perspective" vertical-field-of-view="${Math.PI / 4}"></nve-scene-camera>
