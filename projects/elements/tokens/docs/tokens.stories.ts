  ...
  <table class="tokens-table">
    ...
    <tbody>
      ${Object.entries(tokens).map(([name, value]: any) => {
        ...
        <td>
          ${name}
          <mlv-icon-button style="opacity: 0; margin-top: -18px" title="Copy '${name}' to clipboard" icon-name="copy" interaction="ghost" @click="${() => copyTokenName(name)}"></mlv-icon-button>
        </td>
        <td>
