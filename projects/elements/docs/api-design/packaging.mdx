...
<Meta title="About/API Design/Packaging" />
...
<mlv-alert status="warning">Warning: Avoid alternate build targets as it increases complexity and is unnecessary with modern build tools and browsers.</mlv-alert>

<mlv-alert><mlv-icon slot="icon">🎓</mlv-icon> Learn: <a href="https://justinfagnani.com/2019/11/01/how-to-publish-web-components-to-npm/">Publishing Web Components</a></mlv-alert>
...
```typescript
import  { MlvButton } from '@elements/elements/button';
...
import { MlvDialog } from './dialog.js';

customElements.get('mlv-dialog') || customElements.define('mlv-dialog', MlvDialog);
```
...
<mlv-alert status="warning">Warning: Isolated side-effects are critical for certain micro-frontend use cases.</mlv-alert>
...
<mlv-alert><mlv-icon slot="icon">🎓</mlv-icon> Learn: <a href="./?path=/docs/about-extensions--docs#scoped-registry">Elements Documentation</a></mlv-alert>

<mlv-alert><mlv-icon slot="icon">🎓</mlv-icon> Learn: <a href="https://github.com/webcomponents/polyfills/tree/master/packages/scoped-custom-element-registry">Scoped Element Registry</a></mlv-alert>

<mlv-alert><mlv-icon slot="icon">🎓</mlv-icon> Learn: <a href="https://www.youtube.com/watch?v=QmDToR6mLhk">Case Study</a></mlv-alert>
