  ...
  render() {
    return html`
      <dialog ${animationFade(this)}>
        ${this.closable ? html`<mlv-icon-button @click=${() => this.#typePopoverController.close()} icon-name="cancel" interaction="falt" size="sm" .ariaLabel=${this.i18n.close}></mlv-icon-button>` : ''}
        <slot></slot>
