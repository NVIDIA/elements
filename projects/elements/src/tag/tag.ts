  ...
  render() {
    return html`
      <div internal-host interaction-state focus-within>
        <slot></slot>
        ${this.closable ? html`<nve-icon @click=${() => this.#typeClosableController.close()} interaction="flat" name="cancel" size="sm" role="img" aria-label=${ifDefined(this.i18n.close)}></nve-icon>` : ''}
      </div>
    `;
  }
}
