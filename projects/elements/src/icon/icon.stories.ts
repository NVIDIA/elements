  ...
  render: () => html`
    <div nve-layout="column gap:md">
      <div nve-layout="row align:wrap gap:xs">
        ${GUI_NAMES.map(name => html`<nve-icon name="${`https://brand-assets.cne.ngc.nvidia.com/assets/icons/2.0.1/line/${name}.svg`}" size="lg"></nve-icon>`)}
      </div>
      <nve-divider></nve-divider>
      <div nve-layout="row align:wrap gap:xs">
        ${GUI_NAMES.map(name => html`<nve-icon name="${`https://brand-assets.cne.ngc.nvidia.com/assets/icons/2.0.1/fill/${name}.svg`}" size="lg"></nve-icon>`)}
      </div>
    </div>
  `
}
