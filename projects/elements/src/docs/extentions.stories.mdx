...
It will also register `IconButton` with a new global prefix of `myplugin` rather than the default global `nve-icon-button`.

```html
<myplugin-nve-icon-button icon-name="cancel"></myplugin-nve-icon-button>
```

<Story id="internal-integration--scoped-element" />

While this still creates a global registration for `myplugin-nve-icon-button` its name can be specific to the app, plugin or integration and reduce the chance of a global name collision.