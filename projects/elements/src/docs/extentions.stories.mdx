...
If you are building an extention that cannot leverage Shadow DOM then you can use a prefixing strategy to minimize global tag collisions.
...
It will also register `IconButton` with a new global prefix of `myplugin` rather than the default global `mlv-icon-button`.

```html
<myplugin-mlv-icon-button icon-name="cancel"></myplugin-mlv-icon-button>
```

<Story id="internal-integration--scoped-element" />

While this still creates a global registration for `myplugin-mlv-icon-button` its name can be specific to the app, plugin or integration and reduce the chance of a global name collision.