...
export class Toolbar extends LitElement implements ContainerElement {
  ...
  #updateContainers() {
    if (this.container !== 'flat' && this.container !== 'inset') {
      const groups = this.#slottedElements.filter(e => e.tagName.toLowerCase().includes('nve-button-group')) as ButtonGroup[];
      groups.forEach(group => group.container = 'flat');

      const controls = this.#slottedElements.filter(e => e.hasAttribute('nve-control')) as (Control & { container: string })[];
      controls.forEach(control => control.container = 'flat');

      const buttons= this.#slottedElements.filter(e => e.tagName.toLowerCase().includes('nve-button') || e.tagName.toLowerCase().includes('nve-icon-button')) as (Button | IconButton)[];
