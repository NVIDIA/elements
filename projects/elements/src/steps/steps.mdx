...
The Steps component is composed of a `<nve-steps>` parent element and multiple `<nve-steps-item>` elements slotted as children. <br />
...
In all other examples on this page `behavior-select` is set on the parent `<nve-steps>` to opt into stateful behavior. The following example
