  ...
  <nve-grid>
    ...
    ${getItems().map((row, i) => html`
      <nve-grid-row>
        ${Object.entries(row).map(([, cell]) => html`<nve-grid-cell>${cell.value}</nve-grid-cell> `)}
        <nve-grid-cell>
          <nve-icon-button interaction="ghost" icon-name="additional-actions" aria-label="row ${i} actions"></nve-icon-button>
        </nve-grid-cell>
      </nve-grid-row>
    `)}
  </nve-grid>
...
<div nve-theme="root" nve-layout="column gap:md grow">
  <nve-dropdown closable anchor="column-settings">
    ...
    <nve-button interaction="ghost-destructive" style="--height: initial">restore settings</nve-button>
  </nve-dropdown>
...
class GridPerformanceDemo extends LitElement {
  ...
  render() {
    return html`
      <nve-button @click=${() => this.show = !this.show}>show</nve-button>
      <p>1000 rows, 4000 cells</p>
      ${this.show ? html`
      <nve-grid style="--scroll-height: 400px; width: 1024px">
        <nve-grid-header>
          ${this.grid.columns.map(column => html`<nve-grid-column>${column.label}</nve-grid-column>`)}
        </nve-grid-header>
        ${this.grid.rows.map(row => html`
...
@customElement('nve-grid-panel-demo')
      ...
      <div nve-layout="row gap:md align:stretch" style="height: calc(100% - 28px)">
        <nve-grid>
          ...
          <nve-grid-row ?selected=${this.selectedId === row.id}>
            ${row.cells.map(cell => html`<nve-grid-cell>${cell.label}</nve-grid-cell> `)}
            <nve-grid-cell>
              <nve-icon-button @click=${() => this.selectedId = row.id} interaction="ghost" icon-name="additional-actions"></nve-icon-button>
            </nve-grid-cell>
          </nve-grid-row>`)}
          <nve-grid-footer>
            <nve-icon-button aria-label="show grid options" interaction="ghost" icon-name="additional-actions"></nve-icon-button>
          </nve-grid-footer>
        </nve-grid>
...
export const PanelDetailInteractive = {
  render: () => html`
<div nve-theme="root">
  <nve-grid-panel-demo></nve-grid-panel-demo>
</div>
  `
};
...
<div nve-layout="grid gap:lg span-items:4 pad:md">
  ...
  <div nve-layout="column gap:md align:stretch">
    <h2 nve-text="heading">Column Stack Fixed</h2>
