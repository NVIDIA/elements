...
describe('nve-grid-column', () => {
  ...
  it('should update fixed left column position', async () => {
    ...
    expect(sheets[sheets.length - 1].cssRules[1].cssText.includes('clip-path: inset(0px 0 0px -4px)')).toBe(false);
    expect(sheets[sheets.length - 1].cssRules[1].cssText.includes('--border-right: var(--nve-ref-border-width-sm) solid var(--nve-ref-border-color-muted)')).toBe(true);
  });
...
describe('nve-grid-column actions', () => {
  ...
  beforeEach(async () => {
    ...
    `);
    columns = Array.from(fixture.querySelectorAll('nve-grid-column'));
    actions = Array.from(fixture.querySelectorAll('nve-icon-button'));
    await elementIsStable(columns[0]);
  });
