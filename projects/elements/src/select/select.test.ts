...
describe('nve-select', () => {
  ...
  it('should use aria-hidden for decorative non-semantic icons', async () => {
    const icons = element.shadowRoot.querySelectorAll('nve-icon');
  ...
  it('should update tags when using multiple select and options change', async () => {
    ...
    element.requestUpdate();
    await elementIsStable(element);
    expect(element.shadowRoot.querySelectorAll('nve-tag')[0].innerText).toBe('Option 1');

    select.options[0].innerText = 'Option 1 Updated';
    select.options[0].value = '1-updated';
    await elementIsStable(element);
    expect(element.shadowRoot.querySelectorAll('nve-tag')[0].innerText).toBe('Option 1 Updated');
  });
  ...
  it('should apply disabled styles to tags when disabled with multiple selection', async () => {
    ...
    expect(getComputedStyle(element.shadowRoot.querySelector('nve-tag')).pointerEvents).toBe('auto');
    expect(getComputedStyle(element.shadowRoot.querySelector('nve-tag')).getPropertyValue('--cursor')).toBe('pointer');

    select.disabled = true;
    element.requestUpdate();
    await elementIsStable(element);
    expect(getComputedStyle(element.shadowRoot.querySelector('nve-tag')).pointerEvents).toBe('none');
    expect(getComputedStyle(element.shadowRoot.querySelector('nve-tag')).getPropertyValue('--cursor')).toBe(
      'not-allowed'
    );
  });
