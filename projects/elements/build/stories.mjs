...
export function getStories(filePath) {
  const project = new Project();
  const file = project.addSourceFileAtPath(filePath);
  const stories = file.getChildrenOfKind(SyntaxKind.VariableStatement);
  const element = file.getChildrenOfKind(SyntaxKind.ExportAssignment)[0]
  ...
  return {
    ...
    stories: stories.map(story => {
      ...
      let template = story.getDescendantsOfKind(SyntaxKind.TaggedTemplateExpression)[0]?.getDescendants()[1]?.getText()?.replace(/`/g, '').trim();
      ...
      if (template) {
        ...
        try {
          template = prettier.default.format(template, { parser: 'html', singleAttributePerLine: false, printWidth: 120 })
        } catch {
        }
      }
