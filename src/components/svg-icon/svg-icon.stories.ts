...
export const Default = {
  render: (args: ArgTypes) => html`<nve-svg-icon data-testid="icon" name="${args.name}" variant="${args.variant}" style="font-size: 4em"></nve-svg-icon>`,
...
export const PreviewAllIcons = {
  render: (args: ArgTypes) => html`
    ...
    ${ICON_NAMES.map(iconName => html`
      <span title="${iconName}">
        <nve-svg-icon data-testid="icon" name="${iconName}" variant="${args.variant}" style="font-size: 4em"></nve-svg-icon>
        <!-- ${iconName} -->
      </span>
    `)}
  `,
